...
function FinancePlanner({ accountId, db: db2, userEmail, userName, onSignOut, onUpdateName }) {
  ...
  const addEntry = (entry) => {
    const { fixed, recurrence, destino, goalId, kind, recur, ...rest } = entry;
    if (destino === "guardado") {
...
function EntryModal({ type, expenseCategories, incomeCategories, paymentMethods, goals, initial, onClose, onSave }) {
