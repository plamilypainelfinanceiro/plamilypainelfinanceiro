...
const OVERVIEW_SECTIONS = [
  "Comparado ao m\xEAs passado",
  "A\xE7\xF5es r\xE1pidas",
  "Como ser\xE1 meu m\xEAs?",
  "Insights",
  "Entradas por categoria",
  "Sa\xEDdas por categoria",
  "Entradas x Sa\xEDdas",
  "Fixo x Vari\xE1vel",
  "Evolu\xE7\xE3o do saldo",
  "Sa\xEDdas por pagamento",
  "D\xEDvidas por credor",
  "5 maiores sa\xEDdas",
  "\xDAltimas movimenta\xE7\xF5es",
  "Pr\xF3ximos Compromissos",
  "Metas em progresso"
];
...
function nthBusinessDayOfMonth(y, m, n) {
  let count = 0;
  let d = 1;
  const daysInMonth = new Date(y, m + 1, 0).getDate();
  while (d <= daysInMonth) {
    if (isBusinessDay(y, m, d)) {
      count++;
      if (count === n) return d;
    }
    d++;
...
function generateRecurrenceDates(cfg, startDateStr) {
  const [sy, sm, sd] = startDateStr.split("-").map(Number);
  const startDate = new Date(sy, sm - 1, sd);
  const dates = [];
  const maxSafety = 400;
  const endDate = cfg.endMode === "date" && cfg.endDate ? new Date(cfg.endDate + "T23:59:59") : null;
  const maxCount = cfg.endMode === "count" ? Number(cfg.endCount) || 1 : cfg.endMode === "indefinite" ? 60 : 400;
  if (cfg.frequency === "diaria") {
    let cur = new Date(startDate);
    let i = 0;
    while (dates.length < maxCount && i < maxSafety) {
      if (endDate && cur > endDate) break;
      dates.push(iso(cur.getFullYear(), cur.getMonth(), cur.getDate()));
      cur.setDate(cur.getDate() + 1);
      i++;
    }
  } else if (cfg.frequency === "semanal") {
    const days = cfg.weekdays && cfg.weekdays.length ? cfg.weekdays : [startDate.getDay()];
    let cur = new Date(startDate);
    let i = 0;
    while (dates.length < maxCount && i < maxSafety * 7) {
      if (endDate && cur > endDate) break;
      if (days.includes(cur.getDay())) dates.push(iso(cur.getFullYear(), cur.getMonth(), cur.getDate()));
      cur.setDate(cur.getDate() + 1);
      i++;
    }
  } else if (cfg.frequency === "mensal") {
    let mIdx = 0;
    while (dates.length < maxCount && mIdx < maxSafety) {
      const y = sy + Math.floor((sm - 1 + mIdx) / 12);
      const m = (sm - 1 + mIdx) % 12;
      let day;
      if (cfg.monthlyMode === "businessDay") day = nthBusinessDayOfMonth(y, m, cfg.businessDayN || 5);
      else day = Math.min(cfg.monthDay || sd, new Date(y, m + 1, 0).getDate());
      const dt = new Date(y, m, day);
      if (endDate && dt > endDate) break;
      if (dt >= startDate) dates.push(iso(y, m, day));
      mIdx++;
    }
  } else if (cfg.frequency === "anual") {
    let yIdx = 0;
    while (dates.length < maxCount && yIdx < 60) {
      const y = sy + yIdx;
      const m = (cfg.annualMonth != null ? cfg.annualMonth : sm - 1);
      const day = Math.min(cfg.annualDay || sd, new Date(y, m + 1, 0).getDate());
      const dt = new Date(y, m, day);
      if (endDate && dt > endDate) break;
      if (dt >= startDate) dates.push(iso(y, m, day));
      yIdx++;
...
function SignedBarChartSVG({ data, height = 180 }) {
  ...
  return /* @__PURE__ */ React.createElement("svg", { viewBox: `0 0 ${VB_W} 200`, width: "100%", height, preserveAspectRatio: "xMidYMid meet" }, /* @__PURE__ */ React.createElement("line", { x1: 0, y1: zeroY, x2: VB_W, y2: zeroY, style: { stroke: "var(--border)" }, strokeWidth: "1" }), data.map((d, i) => {
    ...
    return /* @__PURE__ */ React.createElement("g", { key: i }, /* @__PURE__ */ React.createElement("rect", { x: x + barW * 0.25, y, width: barW * 0.5, height: h, rx: "3", fill: color, opacity: "0.85" }), /* @__PURE__ */ React.createElement("text", { x: x + barW * 0.5, y: "180", fontSize: "13", fill: "var(--text-muted)", textAnchor: "middle" }, d.label));
  }));
}
function BarChartSVG({ data, height = 180 }) {
  ...
  const VB_W = 600;
  const gid = useRef("bg" + Math.random().toString(36).slice(2, 8)).current;
  return /* @__PURE__ */ React.createElement("svg", { viewBox: `0 0 ${VB_W} 200`, width: "100%", height, preserveAspectRatio: "xMidYMid meet" }, /* @__PURE__ */ React.createElement("defs", null, /* @__PURE__ */ React.createElement("linearGradient", { id: `${gid}-a`, x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0%", stopColor: c1, stopOpacity: "0.95" }), /* @__PURE__ */ React.createElement("stop", { offset: "100%", stopColor: c1, stopOpacity: "0.65" })), /* @__PURE__ */ React.createElement("linearGradient", { id: `${gid}-b`, x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0%", stopColor: c2, stopOpacity: "0.95" }), /* @__PURE__ */ React.createElement("stop", { offset: "100%", stopColor: c2, stopOpacity: "0.65" }))), data.map((d, i) => {
    const x = i * (VB_W / data.length);
    const barW = VB_W / data.length * 0.32;
    const hE = d.entrada / max * 150;
    const hS = d.saida / max * 150;
    return /* @__PURE__ */ React.createElement("g", { key: i }, /* @__PURE__ */ React.createElement("rect", { x: x + barW * 0.15, y: 160 - hE, width: barW, height: hE, rx: "5", fill: `url(#${gid}-a)` }, /* @__PURE__ */ React.createElement("title", null, `Entradas \u2014 ${d.label}: ${fmt(d.entrada)}`)), /* @__PURE__ */ React.createElement("rect", { x: x + barW * 1.15, y: 160 - hS, width: barW, height: hS, rx: "5", fill: `url(#${gid}-b)` }, /* @__PURE__ */ React.createElement("title", null, `Sa\xEDdas \u2014 ${d.label}: ${fmt(d.saida)}`)), /* @__PURE__ */ React.createElement("text", { x: x + barW * 1.15, y: "180", fontSize: "13", fill: "var(--text-muted)", textAnchor: "middle" }, d.label));
  }));
}
function DonutChartSVG({ data, size = 160 }) {
  ...
  return /* @__PURE__ */ React.createElement("svg", { viewBox: "0 0 160 160", width: size, height: size }, arcs.map((a, i) => /* @__PURE__ */ React.createElement("path", { key: i, d: a.path, fill: "none", stroke: a.d.color, strokeWidth: strokeW, strokeLinecap: "round" }, /* @__PURE__ */ React.createElement("title", null, `${a.d.label}: ${fmt(a.d.value)} (${Math.round(a.d.value / total * 100)}%)`))), /* @__PURE__ */ React.createElement("text", { x: "80", y: "76", textAnchor: "middle", fontSize: "13", fontWeight: "700", fill: "var(--text-dark)" }, fmt(total)), /* @__PURE__ */ React.createElement("text", { x: "80", y: "92", textAnchor: "middle", fontSize: "9", fill: "var(--text-muted)" }, "total"));
}
function LineChartSVG({ points, height = 150, color }) {
  const { c1 } = useTheme();
  const lineColor = color || c1;
  const gid = useRef("lg" + Math.random().toString(36).slice(2, 8)).current;
  ...
  return /* @__PURE__ */ React.createElement("svg", { viewBox: "0 0 300 160", width: "100%", height, preserveAspectRatio: "xMidYMid meet" }, /* @__PURE__ */ React.createElement("defs", null, /* @__PURE__ */ React.createElement("linearGradient", { id: gid, x1: "0", y1: "0", x2: "0", y2: "1" }, /* @__PURE__ */ React.createElement("stop", { offset: "0%", stopColor: lineColor, stopOpacity: "0.28" }), /* @__PURE__ */ React.createElement("stop", { offset: "100%", stopColor: lineColor, stopOpacity: "0" }))), /* @__PURE__ */ React.createElement("path", { d: areaPath, fill: `url(#${gid})` }), /* @__PURE__ */ React.createElement("path", { d: path, fill: "none", stroke: lineColor, strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round" }), coords.map(([x, y], i) => /* @__PURE__ */ React.createElement("circle", { key: i, cx: x, cy: y, r: "3.5", fill: lineColor }, /* @__PURE__ */ React.createElement("title", null, fmt(points[i])))));
}
...
function FinancePlanner({ accountId, db: db2, userEmail, userName, onSignOut, onUpdateName, theme, onSetTheme }) {
  var _a;
  ...
  const exportCSV = () => {
    const rows = [["Data", "Tipo", "Descri\xE7\xE3o", "Categoria", "Forma de pagamento", "Status", "Valor", "Origem"]];
    entries.forEach((e) => {
      rows.push([e.date, e.type === "entrada" ? "Entrada" : "Sa\xEDda", e.desc, e.category, e.method || "", e.status, e.value.toFixed(2).replace(".", ","), "Lan\xE7amento"]);
    });
    purchases.forEach((p) => {
      const card = cards.find((c) => c.id === p.cardId);
      p.installments.forEach((i) => {
        rows.push([i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, "Sa\xEDda", `${p.desc} (parcela ${i.n}/${p.count})`, p.category, `Cart\xE3o${card ? " " + card.name : ""}`, i.status, i.value.toFixed(2).replace(".", ","), "Cart\xE3o de Cr\xE9dito"]);
      });
    });
    debts.forEach((d) => {
      d.installments.forEach((i) => {
        rows.push([i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, "Sa\xEDda", `${d.name} (parcela ${i.n}/${d.count})`, "D\xEDvidas", d.lender || "", i.status, i.value.toFixed(2).replace(".", ","), "D\xEDvida"]);
      });
    });
    savings.forEach((s) => {
      rows.push([s.date, "Guardado", s.desc, "Guardado", s.bank || "", "pago", s.value.toFixed(2).replace(".", ","), "Guardados e Investidos"]);
    });
    investments.forEach((i) => {
      rows.push([i.date, "Investido", i.desc, "Investido", i.bank || "", "pago", i.value.toFixed(2).replace(".", ","), "Guardados e Investidos"]);
    });
    const header = rows[0];
    const dataRows = rows.slice(1).sort((a, b) => (a[0] || "").localeCompare(b[0] || ""));
    const sortedRows = [header, ...dataRows];
    const csvContent = sortedRows.map((r) => r.map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(";")).join("\r\n");
    const blob = new Blob(["\uFEFF" + csvContent], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `plamily-dados-${todayISO()}.csv`;
    a.rel = "noopener";
    ...
    showToast("Dados exportados! Confira sua pasta de downloads.");
  };
  ...
  const [savingPots, setSavingPots] = useStoredState(accountId, "savingPots", []);
  const [investPots, setInvestPots] = useStoredState(accountId, "investPots", []);
  ...
  const [openingBalance, setOpeningBalance] = useStoredState(accountId, "openingBalance", 0);
  ...
  const [banks, setBanks] = useStoredState(accountId, "banks", BANKS);
  const saveTimer = useRef(null);
  const firstRun = useRef(true);
  useEffect(() => {
    const data = { entries, cards, purchases, debts, wishlist, goals, savings, investments, savingPots, investPots, budgets, expenseCategories, incomeCategories, paymentMethods, debtTypes, categoryColors, profilePhoto, openingBalance, theme, banks };
  ...
  }, [entries, cards, purchases, debts, wishlist, goals, savings, investments, savingPots, investPots, budgets, expenseCategories, incomeCategories, paymentMethods, debtTypes, categoryColors, profilePhoto, openingBalance, theme, banks]);
  ...
  const cardInvoices = useMemo(() => {
    ...
    allInstallmentsFlat.forEach((i) => {
      if (!byCard[i.cardId]) return;
      const key = i.cardId + "|" + i.dueKey;
      if (!byCardKey[key]) {
        byCardKey[key] = { cardId: i.cardId, dueKey: i.dueKey, dueY: i.dueY, dueM: i.dueM, dueDate: i.dueDate, items: [] };
        byCard[i.cardId].push(byCardKey[key]);
      }
      byCardKey[key].items.push(i);
    });
    Object.keys(byCard).forEach((cardId) => {
      byCard[cardId] = byCard[cardId].map((inv) => {
        const pending = inv.items.filter((i) => i.status === "pendente");
        return {
          ...inv,
          total: inv.items.reduce((s, i) => s + i.value, 0),
          pendingTotal: pending.reduce((s, i) => s + i.value, 0),
  ...
  const totals = useMemo(() => {
    ...
    const saldo = (Number(openingBalance) || 0) + paidIn - paidOut - paidInstallments - paidDebtInstallments - savingsDiscount - investDiscount;
    ...
    const saldoMes = (Number(openingBalance) || 0) + monthIn - monthOut - monthSavingsDiscount - monthInvestDiscount;
    return {
      saldo,
      saldoMes,
      monthIn,
  ...
  }, [entries, allInstallmentsFlat, debtInstallmentsFlat, savings, investments, selY, selM, selKey, openingBalance]);
  const fixedVsVariable = useMemo(() => {
    const monthSaidas = entries.filter((e) => e.type === "saida" && e.status === "pago" && dateInMonth(e.date, selY, selM));
    ...
    const monthEntradas = entries.filter((e) => e.type === "entrada" && e.status === "pago" && dateInMonth(e.date, selY, selM));
    const fixedInTotal = monthEntradas.filter((e) => e.fixed).reduce((s, e) => s + e.value, 0);
    const variableInTotal = monthEntradas.filter((e) => !e.fixed).reduce((s, e) => s + e.value, 0);
    return { fixedTotal, variableTotal, fixedInTotal, variableInTotal };
  }, [entries, selY, selM]);
  const monthConquests = useMemo(() => {
    const bought = wishlist.filter((w) => w.bought && w.boughtDate);
    const monthPrefix = `${selY}-${pad2(selM + 1)}`;
    const items = bought.filter((w) => w.boughtDate.startsWith(monthPrefix));
    return { items, total: items.reduce((s, w) => s + w.value, 0) };
  }, [wishlist, selY, selM]);
  ...
  const monthProjection = useMemo(() => {
    ...
    return {
      ...
      projecao: (Number(openingBalance) || 0) + previstoTotal - comprometidoTotal
    };
  ...
  const previousMonthCategorySpend = useMemo(() => {
    ...
    const map = {};
    entries.filter((e) => e.type === "saida" && e.status === "pago" && dateInMonth(e.date, y, m)).forEach((e) => {
  ...
  const paymentMethodSpend = useMemo(() => {
    const map = {};
    entries.filter((e) => e.type === "saida" && e.status === "pago" && dateInMonth(e.date, selY, selM)).forEach((e) => {
      map[e.method] = (map[e.method] || 0) + e.value;
    });
    allInstallmentsFlat.filter((i) => i.status === "pago" && i.dueKey === selKey).forEach((i) => {
      map["Cr\xE9dito"] = (map["Cr\xE9dito"] || 0) + i.value;
    });
  ...
  }, [entries, allInstallmentsFlat, selY, selM, selKey]);
  ...
  const categorySpendMonth = useMemo(() => {
    ...
    allInstallmentsFlat.filter((i) => i.dueKey === selKey && i.status === "pago").forEach((i) => {
      map[i.category] = (map[i.category] || 0) + i.value;
    });
    debtInstallmentsFlat.filter((i) => i.dueKey === selKey && i.status === "pago").forEach((i) => {
      map[i.category] = (map[i.category] || 0) + i.value;
    });
  ...
  const top5Saidas = useMemo(() => {
    const fromEntries = entries.filter((e) => e.type === "saida" && e.status === "pago" && dateInMonth(e.date, selY, selM)).map((e) => ({ id: e.id, desc: e.desc, category: e.category, value: e.value }));
    const fromCards = allInstallmentsFlat.filter((i) => i.status === "pago" && i.dueKey === selKey).map((i) => ({ id: i.purchaseId + "-" + i.n, desc: `${i.desc} (parcela ${i.n})`, category: i.category, value: i.value }));
    const fromDebts = debtInstallmentsFlat.filter((i) => i.status === "pago" && i.dueKey === selKey).map((i) => ({ id: i.debtId + "-" + i.n, desc: `${i.desc} (parcela ${i.n})`, category: i.category, value: i.value }));
    return [...fromEntries, ...fromCards, ...fromDebts].sort((a, b) => b.value - a.value).slice(0, 5);
  }, [entries, allInstallmentsFlat, debtInstallmentsFlat, selY, selM, selKey]);
  const incomeByCategory = useMemo(() => {
    const map = {};
    entries.filter((e) => e.type === "entrada" && e.status === "pago" && dateInMonth(e.date, selY, selM)).forEach((e) => {
      map[e.category] = (map[e.category] || 0) + e.value;
    });
  ...
  const monthComparison = useMemo(() => {
    ...
    const prevEntries = entries.filter((e) => e.status === "pago" && dateInMonth(e.date, py, pm));
    ...
    if (totals.monthInPending > 0 || totals.monthOutPendingEntries > 0) {
      msgs.push(`Isso considera s\xF3 o que j\xE1 foi pago. Ainda h\xE1 ${fmt(totals.monthInPending + totals.monthOutPendingEntries)} pendente que pode mudar esse panorama.`);
    }
    if (msgs.length === 0) msgs.push("Ainda n\xE3o h\xE1 dados suficientes para comparar este per\xEDodo.");
    return { hasData: true, msgs: msgs.slice(0, 4) };
  ...
  const balanceEvolution = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      arr.push((Number(openingBalance) || 0) + inn - out - inst - debtInst);
    }
    return arr;
  }, [entries, allInstallmentsFlat, debtInstallmentsFlat, selY, selM, openingBalance]);
  const upcoming = useMemo(() => {
    const list = [];
    entries.filter((e) => e.status === "pendente").forEach((e) => list.push({ id: e.id, desc: e.desc, value: e.value, date: e.date, type: e.type, kind: e.type === "entrada" ? "A receber" : "Conta", source: "entry", rawId: e.id }));
    allInstallmentsFlat.filter((i) => i.status === "pendente").forEach(
      (i) => list.push({ id: i.purchaseId + i.n, desc: `${i.desc} (parcela ${i.n})`, value: i.value, date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", kind: "Cart\xE3o", source: "purchase", rawId: i.purchaseId, n: i.n })
    );
    debtInstallmentsFlat.filter((i) => i.status === "pendente").forEach(
      (i) => list.push({ id: i.debtId + i.n, desc: `${i.desc} (parcela ${i.n})`, value: i.value, date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", kind: "D\xEDvida", source: "debt", rawId: i.debtId, n: i.n })
    );
  ...
  const dueSoon = useMemo(() => {
    ...
    allInstallmentsFlat.filter((i) => i.status === "pendente").forEach((i) => {
      const d = i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`;
  ...
  const recentMovements = useMemo(() => {
    const fromEntries = entries.filter((e) => dateInMonth(e.date, selY, selM)).map((e) => ({ id: e.id, date: e.date, type: e.type, desc: e.desc, category: e.category, value: e.value, status: e.status }));
    const fromCards = allInstallmentsFlat.filter((i) => i.dueKey === selKey).map((i) => ({ id: i.purchaseId + "-" + i.n, date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", desc: `${i.desc} (parcela ${i.n})`, category: i.category, value: i.value, status: i.status }));
    const fromDebts = debtInstallmentsFlat.filter((i) => i.dueKey === selKey).map((i) => ({ id: i.debtId + "-" + i.n, date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", desc: `${i.desc} (parcela ${i.n})`, category: i.category, value: i.value, status: i.status }));
    return [...fromEntries, ...fromCards, ...fromDebts].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 6);
  }, [entries, allInstallmentsFlat, debtInstallmentsFlat, selY, selM, selKey]);
  const addTransfer = (t) => {
    const transferId = uid();
    const desc = `Transfer\xEAncia: ${t.fromBank} \u2192 ${t.toBank}`;
    setEntries((prev) => [
      { id: uid(), type: "saida", desc, category: "Transfer\xEAncia entre contas", value: t.value, date: t.date, method: "Transfer\xEAncia", bank: t.fromBank, status: "pago", fixed: false, isTransfer: true, transferId },
      { id: uid(), type: "entrada", desc, category: "Transfer\xEAncia entre contas", value: t.value, date: t.date, method: "Transfer\xEAncia", bank: t.toBank, status: "pago", fixed: false, isTransfer: true, transferId },
      ...prev
    ]);
    showToast("Transfer\xEAncia registrada");
  };
  const addEntry = (entry) => {
    const { fixed, recurrence, destino, goalId, kind, recur, cardId, ...rest } = entry;
    if (cardId) {
      if (kind === "recorrente") {
        const dates = generateRecurrenceDates(recur, rest.date);
        dates.forEach((dateISO) => {
          addPurchase({ cardId, desc: rest.desc, category: rest.category, total: rest.value, count: 1, purchaseDate: dateISO });
        });
        showToast(`Assinatura recorrente criada no cart\xE3o (${dates.length} cobran\xE7as)`);
        return;
      }
      const count = kind === "parcelada" ? Number(recur.installmentCount) || 1 : 1;
      addPurchase({ cardId, desc: rest.desc, category: rest.category, total: rest.value * count, count, purchaseDate: rest.date });
      return;
    }
    if (destino === "guardado") {
      setSavings((prev) => [{ id: uid(), desc: rest.desc, value: rest.value, bank: rest.bank, yieldRate: null, date: rest.date, discount: false, viaEntry: true, potId: goalId || void 0 }, ...prev]);
    } else if (destino === "investimento") {
      setInvestments((prev) => [{ id: uid(), desc: rest.desc, value: rest.value, bank: rest.bank, yieldRate: null, date: rest.date, discount: false, viaEntry: true, potId: goalId || void 0 }, ...prev]);
    } else if (destino === "meta" && goalId) {
      setGoals((prev) => prev.map((g) => g.id === goalId ? { ...g, current: g.current + rest.value, history: [...g.history || [], { date: rest.date, value: rest.value }] } : g));
    } else if (destino === "retirar_guardado" && goalId) {
      setSavings((prev) => [{ id: uid(), desc: `Retirada: ${rest.desc}`, value: -rest.value, bank: rest.bank, yieldRate: null, date: rest.date, discount: false, viaEntry: true, potId: goalId }, ...prev]);
    } else if (destino === "retirar_investimento" && goalId) {
      setInvestments((prev) => [{ id: uid(), desc: `Retirada: ${rest.desc}`, value: -rest.value, bank: rest.bank, yieldRate: null, date: rest.date, discount: false, viaEntry: true, potId: goalId }, ...prev]);
    }
    rest.goalId = destino === "meta" ? goalId : void 0;
    if (kind === "parcelada" || kind === "recorrente") {
      const cfg = kind === "parcelada" ? { frequency: "mensal", monthlyMode: "fixedDay", monthDay: Number(entry.date.split("-")[2]), endMode: "count", endCount: recur.installmentCount } : recur;
      const dates = generateRecurrenceDates(cfg, entry.date);
      const recurrenceId = uid();
      const generated = dates.map((dateISO, i) => ({
        id: uid(),
        ...rest,
        date: dateISO,
        ...
        recurrenceKind: kind,
        recurrenceCfg: cfg,
        unlimited: kind === "recorrente" && cfg.endMode === "indefinite"
      }));
      setEntries((prev) => [...generated, ...prev]);
      showToast(
        kind === "parcelada" ? `Sa\xEDda parcelada em ${dates.length}x criada` : cfg.endMode === "indefinite" ? "Lan\xE7amento recorrente criado sem data de t\xE9rmino" : `Lan\xE7amento recorrente criado (${dates.length} ocorr\xEAncias)`
      );
    } else {
  ...
  const editEntry = (id, updated) => {
    ...
    if (prevEntry && prevEntry.goalId && rest.value !== void 0 && rest.value !== prevEntry.value) {
      const diff = rest.value - prevEntry.value;
      setGoals((prevGoals) => prevGoals.map((g) => g.id === prevEntry.goalId ? { ...g, current: g.current + diff, history: (g.history || []).map((h) => h.date === prevEntry.date && h.value === prevEntry.value ? { ...h, value: rest.value } : h) } : g));
    }
  ...
  const deleteEntry = (id) => {
    ...
    if (target && target.goalId) {
      setGoals((prevGoals) => prevGoals.map((g) => g.id === target.goalId ? { ...g, current: Math.max(0, g.current - target.value), history: (g.history || []).filter((h) => !(h.date === target.date && h.value === target.value)) } : g));
    }
    setEntries((prev) => prev.filter((e) => e.id !== id));
  };
  const stopRecurrence = (recurrenceId) => {
    const todayStr = todayISO();
    setEntries((prev) => prev.filter((e) => !(e.recurrenceId === recurrenceId && e.status === "pendente" && e.date >= todayStr)));
    showToast("Recorr\xEAncia encerrada. O que j\xE1 aconteceu continua no Hist\xF3rico.");
  };
  const addCard = (card) => {
  ...
  const addPurchase = (p) => {
    const [y, m, d] = p.purchaseDate.split("-").map(Number);
    const card = cards.find((c) => c.id === p.cardId);
    let startY = y, startM = m - 1;
    if (card && card.closing && d > card.closing) {
      startM += 1;
      if (startM > 11) {
        startM = 0;
        startY += 1;
    ...
    const installments = buildInstallments(p.total, p.count, startY, startM, card ? card.due : null);
  ...
  const payInvoice = (cardId, dueKey) => {
    setPurchases(
      (prev) => prev.map(
        (p) => p.cardId === cardId ? { ...p, installments: p.installments.map((i) => i.dueKey === dueKey && i.status === "pendente" ? { ...i, status: "pago" } : i) } : p
      )
    );
    showToast("Fatura marcada como paga");
  };
  const reopenInvoice = (cardId, dueKey) => {
    setPurchases(
      (prev) => prev.map(
        (p) => p.cardId === cardId ? { ...p, installments: p.installments.map((i) => i.dueKey === dueKey && i.status === "pago" ? { ...i, status: "pendente" } : i) } : p
      )
    );
  ...
  const addSavingPot = (name) => {
    const clean = name.trim();
    if (!clean) return null;
    const id = uid();
    setSavingPots((prev) => [...prev, { id, name: clean }]);
    return id;
  };
  const deleteSavingPot = (id) => {
    setSavingPots((prev) => prev.filter((p) => p.id !== id));
    setSavings((prev) => prev.map((s) => s.potId === id ? { ...s, potId: void 0 } : s));
    showToast("Cofrinho exclu\xEDdo (o hist\xF3rico continua salvo, sem cofrinho)");
  };
  const addInvestPot = (name) => {
    const clean = name.trim();
    if (!clean) return null;
    const id = uid();
    setInvestPots((prev) => [...prev, { id, name: clean }]);
    return id;
  };
  const deleteInvestPot = (id) => {
    setInvestPots((prev) => prev.filter((p) => p.id !== id));
    setInvestments((prev) => prev.map((i) => i.potId === id ? { ...i, potId: void 0 } : i));
    showToast("Cofrinho exclu\xEDdo (o hist\xF3rico continua salvo, sem cofrinho)");
  };
  ...
  const searchResults = useMemo(() => {
    ...
    OVERVIEW_SECTIONS.forEach((label) => {
      if (label.toLowerCase().includes(q)) out.push({ kind: "Vis\xE3o Geral", label, sub: "ir para a tela inicial", view: "overview" });
    });
    entries.forEach((e) => {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "app-root" }, /* @__PURE__ */ React.createElement("style", null, CSS), /* @__PURE__ */ React.createElement("aside", { className: "sidebar" }, /* @__PURE__ */ React.createElement("div", { className: "brand" }, /* @__PURE__ */ React.createElement("div", { className: "brand-mark" }, /* @__PURE__ */ React.createElement("img", { src: "./icon-192.png", alt: "Plamily", className: "brand-mark-img" })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "brand-title" }, "Plamily"), /* @__PURE__ */ React.createElement("div", { className: "brand-sub" }, "painel financeiro"))), /* @__PURE__ */ React.createElement("nav", { className: "side-nav" }, NAV.map((n) => /* @__PURE__ */ React.createElement(
    ...
    OverviewView,
    {
      ...
      onToggleEntry: toggleEntryStatus,
      onToggleInstallment: toggleInstallmentStatus,
      onToggleDebtInstallment: toggleDebtInstallmentStatus
    }
  ), view === "annual" && /* @__PURE__ */ React.createElement(AnnualView, { entries, allInstallmentsFlat, debtInstallmentsFlat, selY }), view === "entries" && /* @__PURE__ */ React.createElement(
    EntriesView,
    {
  ...
  ), view === "calendar" && /* @__PURE__ */ React.createElement(CalendarView, { key: resetTick, entries, allInstallmentsFlat, debtInstallmentsFlat, expenseCategories, incomeCategories, categoryColors }), view === "budget" && /* @__PURE__ */ React.createElement(BudgetView, { budgets, spend: categorySpendMonth, onUpdate: updateBudget, categories: expenseCategories, categoryColors }), view === "savingsinvest" && /* @__PURE__ */ React.createElement(SavingsInvestView, { savings, investments, savingPots, investPots, onEditSaving: editSaving, onDeleteSaving: deleteSaving, onEditInvestment: editInvestment, onDeleteInvestment: deleteInvestment, onDeletePot: (id) => { if (savingPots.some((p) => p.id === id)) deleteSavingPot(id); else deleteInvestPot(id); }, openModal: setModal }), view === "cards" && /* @__PURE__ */ React.createElement(CardsView, { cards, purchases, cardInvoices, onToggleInstallment: toggleInstallmentStatus, onPayInvoice: payInvoice, onReopenInvoice: reopenInvoice, openModal: setModal }), view === "debts" && /* @__PURE__ */ React.createElement(DebtsView, { debtsSummary, onToggleInstallment: toggleDebtInstallmentStatus, onDelete: deleteDebt, openModal: setModal, debtTypes }), view === "wishlist" && /* @__PURE__ */ React.createElement(WishlistView, { wishlist, onDelete: deleteWish, openModal: setModal, setModal }), view === "goals" && /* @__PURE__ */ React.createElement(GoalsView, { goals, onDelete: deleteGoal, openModal: setModal }), view === "history" && /* @__PURE__ */ React.createElement(HistoryView, { entries, purchases, debts, savings, investments, expenseCategories, incomeCategories, categoryColors }), view === "categories" && /* @__PURE__ */ React.createElement(CategoriesAndCardsView, { cards, expenseCategories, incomeCategories, categoryColors, paymentMethods, debtTypes, banks, onAddCategory: addCategory, onDeleteCategory: deleteCategory, onRenameCategory: renameCategory, onSetCategoryColor: setCategoryColor, onEditCard: editCard, onDeleteCard: deleteCard, openModal: setModal }), view === "tips" && /* @__PURE__ */ React.createElement(TipsView, { totals, categorySpendMonth, budgets }), view === "settings" && /* @__PURE__ */ React.createElement(
    SettingsView,
    {
    ...
    addPurchase(p);
    setModal(null);
  } }), modal === "addCard" && /* @__PURE__ */ React.createElement(CardModal, { banks, onClose: () => setModal(null), onSave: (c) => {
    addCard(c);
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) === "editCard" && /* @__PURE__ */ React.createElement(CardModal, { card: modal.card, banks, onClose: () => setModal(null), onSave: (updates) => {
    ...
    setModal(null);
  } }), modal === "addDebt" && /* @__PURE__ */ React.createElement(DebtModal, { debtTypes, banks, onClose: () => setModal(null), onSave: (d) => {
    addDebt(d);
    setModal(null);
    ...
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) === "editDebt" && /* @__PURE__ */ React.createElement(EditDebtModal, { debt: modal.debt, debtTypes, banks, onClose: () => setModal(null), onSave: (updates) => {
    ...
    addWish(w);
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) === "editWish" && /* @__PURE__ */ React.createElement(WishModal, { initial: modal.wish, categories: expenseCategories, onClose: () => setModal(null), onSave: (w) => {
    editWish(modal.wish.id, w);
    setModal(null);
  } }), modal === "addTransfer" && /* @__PURE__ */ React.createElement(TransferModal, { banks, onClose: () => setModal(null), onSave: (t) => {
    addTransfer(t);
    setModal(null);
  } }), modal === "addGoal" && /* @__PURE__ */ React.createElement(GoalModal, { onClose: () => setModal(null), onSave: (g) => {
    addGoal(g);
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) === "editGoal" && /* @__PURE__ */ React.createElement(GoalModal, { initial: modal.goal, onClose: () => setModal(null), onSave: (g) => {
    editGoal(modal.goal.id, g);
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) && (modal.kind === "confirmDelete" || modal.kind === "confirmReset") && /* @__PURE__ */ React.createElement(ConfirmDangerousModal, { action: modal.kind === "confirmDelete" ? "delete" : "reset", userEmail, onClose: () => setModal(null), onConfirm: modal.kind === "confirmDelete" ? performDeleteAccount : performResetAccount }), modal === "addSaving" && /* @__PURE__ */ React.createElement(SavingModal, { banks, pots: savingPots, onAddPot: addSavingPot, onClose: () => setModal(null), onSave: (s) => {
    addSaving(s);
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) === "editSaving" && /* @__PURE__ */ React.createElement(SavingModal, { initial: modal.item, banks, pots: savingPots, onAddPot: addSavingPot, onClose: () => setModal(null), onSave: (s) => {
    editSaving(modal.item.id, s);
    setModal(null);
  } }), modal === "addInvestment" && /* @__PURE__ */ React.createElement(InvestmentModal, { banks, pots: investPots, onAddPot: addInvestPot, onClose: () => setModal(null), onSave: (i) => {
    addInvestment(i);
    setModal(null);
  } }), typeof modal === "object" && (modal == null ? void 0 : modal.kind) === "editInvestment" && /* @__PURE__ */ React.createElement(InvestmentModal, { initial: modal.item, banks, pots: investPots, onAddPot: addInvestPot, onClose: () => setModal(null), onSave: (i) => {
...
function OverviewView({ totals, categorySpendMonth, bars, balanceEvo, paymentMethodSpend, debtsSummary, upcoming, recent, goals, fixedVsVariable, openModal, userName, selY, selM, selDay, daySummary, budgets, monthProjection, safeLimit, smartInsights, top5Saidas, incomeByCategory, monthComparison, dueSoon, monthConquests, openingBalance, onSetOpeningBalance, onToggleEntry, onToggleInstallment, onToggleDebtInstallment }) {
  ...
  const greeting = firstName ? `Ol\xE1 ${firstName}, ${timeGreeting}!` : `Ol\xE1, ${timeGreeting}!`;
...
function AnnualView({ entries, allInstallmentsFlat, debtInstallmentsFlat, selY }) {
  const monthData = useMemo(() => {
    const arr = [];
    for (let m = 0; m < 12; m++) {
      const prefix = `${selY}-${pad2(m + 1)}`;
      const monthKey = `${selY}-${pad2(m + 1)}`;
      const monthEntries = entries.filter((e) => e.date && e.date.startsWith(prefix));
      const totalInEntries = monthEntries.filter((e) => e.type === "entrada" && e.status === "pago").reduce((s, e) => s + e.value, 0);
      const totalOutEntries = monthEntries.filter((e) => e.type === "saida" && e.status === "pago").reduce((s, e) => s + e.value, 0);
      const totalOutCards = allInstallmentsFlat.filter((i) => i.status === "pago" && i.dueKey === monthKey).reduce((s, i) => s + i.value, 0);
      const totalOutDebts = debtInstallmentsFlat.filter((i) => i.status === "pago" && i.dueKey === monthKey).reduce((s, i) => s + i.value, 0);
      const totalIn = totalInEntries;
      const totalOut = totalOutEntries + totalOutCards + totalOutDebts;
      const hasData = monthEntries.length > 0 || allInstallmentsFlat.some((i) => i.dueKey === monthKey) || debtInstallmentsFlat.some((i) => i.dueKey === monthKey);
      arr.push({ m, totalIn, totalOut, net: totalIn - totalOut, hasData });
    }
    return arr;
  }, [entries, allInstallmentsFlat, debtInstallmentsFlat, selY]);
...
function CalendarView({ entries, allInstallmentsFlat, debtInstallmentsFlat, expenseCategories, incomeCategories, categoryColors }) {
  ...
  const installmentsAsEntries = [
    ...allInstallmentsFlat.map((i) => ({ id: i.purchaseId + "-" + i.n, date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", desc: `${i.desc} (parcela ${i.n})`, category: i.category, value: i.value })),
    ...debtInstallmentsFlat.map((i) => ({ id: i.debtId + "-" + i.n, date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", desc: `${i.desc} (parcela ${i.n})`, category: i.category, value: i.value }))
  ];
  const allForCalendar = [...entries, ...installmentsAsEntries];
  const dayMap = {};
  allForCalendar.forEach((e) => {
    if (!e.date) return;
  ...
  const selectedEntries = selectedDate ? allForCalendar.filter((e) => e.date === selectedDate) : [];
...
function EntriesView({ entries, onToggle, onDelete, onStopRecurrence, openModal }) {
...
function CardsView({ cards, purchases, cardInvoices, onToggleInstallment, onPayInvoice, onReopenInvoice, openModal }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "stack-lg" }, /* @__PURE__ */ React.createElement("div", { className: "filter-row" }, /* @__PURE__ */ React.createElement("div", { className: "spacer" }), /* @__PURE__ */ React.createElement("button", { className: "btn-secondary sm", onClick: () => openModal("addCard") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), "Novo cart\xE3o"), /* @__PURE__ */ React.createElement("button", { className: "btn-primary sm", onClick: () => openModal("addPurchase") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), "Compra")), /* @__PURE__ */ React.createElement("div", { className: "cards-row" }, cards.map((c) => {
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { className: "cc-limit-row" }, /* @__PURE__ */ React.createElement("span", null, fmt(used), " de ", fmt(c.limit), " do limite"), /* @__PURE__ */ React.createElement("strong", null, Math.round(pct), "%")), /* @__PURE__ */ React.createElement("div", { className: "cc-limit-bar" }, /* @__PURE__ */ React.createElement("div", { style: { width: `${pct}%` } }))
    );
  })), /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "Faturas"), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, selectedCard ? "cart\xE3o selecionado" : "todos os cart\xF5es")), (() => {
  const cardIds = selectedCard ? [selectedCard] : cards.map((c) => c.id);
  const rows = [];
  cardIds.forEach((cid) => {
    const card = cards.find((c) => c.id === cid);
    const invs = (cardInvoices[cid] || []).filter((inv) => inv.itemCount > 0);
    invs.forEach((inv) => rows.push({ card, inv }));
  });
  rows.sort((a, b) => a.inv.dueKey.localeCompare(b.inv.dueKey));
  if (rows.length === 0) return /* @__PURE__ */ React.createElement(EmptyState, { icon: /* @__PURE__ */ React.createElement(CreditCard, { size: 24 }), text: "Nenhuma fatura por aqui ainda." });
  return /* @__PURE__ */ React.createElement("div", { className: "invoice-list" }, rows.map(({ card, inv }) => {
  const todayStr2 = todayISO();
  const isOverdue = !inv.isFullyPaid && inv.dueDate && inv.dueDate < todayStr2;
  const statusTxt = inv.isFullyPaid ? " \xB7 paga" : isOverdue ? " \xB7 vencida" : ` \xB7 ${inv.pendingCount} pendente${inv.pendingCount > 1 ? "s" : ""}`;
  return /* @__PURE__ */ React.createElement("div", { key: card.id + inv.dueKey, className: `invoice-row ${isOverdue ? "invoice-overdue" : ""}` }, /* @__PURE__ */ React.createElement("div", { className: "invoice-info" }, /* @__PURE__ */ React.createElement("span", { className: "invoice-card-name" }, card.name), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, MONTHS[inv.dueM], "/", inv.dueY, " \xB7 ", inv.itemCount, " item", inv.itemCount > 1 ? "s" : "", statusTxt)), /* @__PURE__ */ React.createElement("div", { className: "invoice-actions" }, /* @__PURE__ */ React.createElement("strong", null, fmt(inv.isFullyPaid ? inv.total : inv.pendingTotal)), inv.isFullyPaid ? /* @__PURE__ */ React.createElement("button", { className: "chip static", onClick: () => onReopenInvoice(card.id, inv.dueKey) }, "Reabrir fatura") : /* @__PURE__ */ React.createElement("button", { className: "btn-primary sm", onClick: () => onPayInvoice(card.id, inv.dueKey) }, "Pagar fatura")));
  }));
...
function SavingsInvestView({ savings, investments, savingPots, investPots, onEditSaving, onDeleteSaving, onEditInvestment, onDeleteInvestment, onDeletePot, openModal }) {
  const [expanded, setExpanded] = useState({});
  const totalS = savings.reduce((s, x) => s + x.value, 0);
  const totalI = investments.reduce((s, x) => s + x.value, 0);
  const renderRow = (item, onEdit, onDelete) => /* @__PURE__ */ React.createElement("div", { key: item.id, className: "sav-inv-row" }, /* @__PURE__ */ React.createElement("div", { className: "sav-inv-info" }, /* @__PURE__ */ React.createElement("strong", null, item.desc), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, item.bank || "\u2014", item.yieldRate ? ` \xB7 ${item.yieldRate}% a.m.` : "", " \xB7 ", new Date(item.date + "T00:00:00").toLocaleDateString("pt-BR"), item.viaEntry ? " \xB7 via Lan\xE7amentos" : item.discount === false ? " \xB7 n\xE3o descontado do saldo" : "")), /* @__PURE__ */ React.createElement("div", { className: "sav-inv-actions" }, /* @__PURE__ */ React.createElement("strong", { className: item.value < 0 ? "value-negative" : "" }, fmt(item.value)), /* @__PURE__ */ React.createElement("button", { className: "icon-btn", title: "Editar", onClick: () => onEdit(item) }, /* @__PURE__ */ React.createElement(Edit3, { size: 13 })), /* @__PURE__ */ React.createElement("button", { className: "icon-btn danger", title: "Excluir", onClick: () => onDelete(item.id) }, /* @__PURE__ */ React.createElement(Trash2, { size: 13 }))));
  const renderPotGroup = (kind, pots, items, onEdit, onDelete) => {
    const grouped = pots.map((p) => ({ pot: p, items: items.filter((it) => it.potId === p.id) }));
    const loose = items.filter((it) => !it.potId || !pots.some((p) => p.id === it.potId));
    return /* @__PURE__ */ React.createElement(React.Fragment, null, grouped.map(({ pot, items: potItems }) => {
      const subtotal = potItems.reduce((s, x) => s + x.value, 0);
      const key = kind + pot.id;
      const isOpen = !!expanded[key];
      return /* @__PURE__ */ React.createElement("div", { key: pot.id, className: "pot-card" }, /* @__PURE__ */ React.createElement("button", { className: "pot-head", onClick: () => setExpanded((prev) => ({ ...prev, [key]: !prev[key] })) }, /* @__PURE__ */ React.createElement("span", { className: "pot-name" }, /* @__PURE__ */ React.createElement(PiggyBank, { size: 15 }), pot.name, /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, " \xB7 ", potItems.length, " movimento", potItems.length !== 1 ? "s" : "")), /* @__PURE__ */ React.createElement("span", { className: "pot-total" }, fmt(subtotal), /* @__PURE__ */ React.createElement(ChevronDown, { size: 14, style: { transform: isOpen ? "rotate(180deg)" : "none" } }))), /* @__PURE__ */ React.createElement("div", { className: "pot-actions-row" }, /* @__PURE__ */ React.createElement("button", { className: "icon-btn danger", title: "Excluir cofrinho", onClick: () => { if (window.confirm(`Excluir o cofrinho "${pot.name}"? O hist\xF3rico continua salvo, s\xF3 sem cofrinho vinculado.`)) onDeletePot(pot.id); } }, /* @__PURE__ */ React.createElement(Trash2, { size: 12 }))), isOpen && (potItems.length === 0 ? /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Nenhum movimento neste cofrinho ainda.") : /* @__PURE__ */ React.createElement("div", { className: "sav-inv-list" }, potItems.map((it) => renderRow(it, onEdit, onDelete)))));
    }), loose.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "sav-inv-list" }, loose.map((it) => renderRow(it, onEdit, onDelete))));
  };
  return /* @__PURE__ */ React.createElement("div", { className: "stack-lg" }, /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "Guardado"), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, fmt(totalS), " no total")), /* @__PURE__ */ React.createElement("button", { className: "btn-secondary sm", style: { marginBottom: 14 }, onClick: () => openModal("addSaving") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), "Novo guardado"), savingPots.length === 0 && savings.length === 0 ? /* @__PURE__ */ React.createElement(EmptyState, { icon: /* @__PURE__ */ React.createElement(PiggyBank, { size: 24 }), text: "Nenhum cofrinho criado ainda. Toque em \u201CNovo guardado\u201D pra come\xE7ar." }) : renderPotGroup("s", savingPots, savings, (it) => openModal({ kind: "editSaving", item: it }), onDeleteSaving)), /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "Investido"), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, fmt(totalI), " no total")), /* @__PURE__ */ React.createElement("button", { className: "btn-secondary sm", style: { marginBottom: 14 }, onClick: () => openModal("addInvestment") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), "Novo investimento"), investPots.length === 0 && investments.length === 0 ? /* @__PURE__ */ React.createElement(EmptyState, { icon: /* @__PURE__ */ React.createElement(TrendingUp, { size: 24 }), text: "Nenhum cofrinho criado ainda. Toque em \u201CNovo investimento\u201D pra come\xE7ar." }) : renderPotGroup("i", investPots, investments, (it) => openModal({ kind: "editInvestment", item: it }), onDeleteInvestment)));
}
...
function WishlistView({ wishlist, onDelete, setModal }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "stack-lg" }, conquistasMes.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "card conquest-card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "Conquistas deste m\xEAs"), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, conquistasMes.length, " item", conquistasMes.length > 1 ? "s" : "", " \xB7 ", fmt(totalConquistas))), /* @__PURE__ */ React.createElement("div", { className: "conquest-list" }, conquistasMes.map((w) => /* @__PURE__ */ React.createElement("div", { key: w.id, className: "conquest-item" }, /* @__PURE__ */ React.createElement(Check, { size: 14 }), /* @__PURE__ */ React.createElement("span", null, w.name), /* @__PURE__ */ React.createElement("strong", null, fmt(w.value))))), /* @__PURE__ */ React.createElement("p", { className: "muted-sm", style: { marginTop: 10 } }, "Parab\xE9ns! Voc\xEA realizou ", conquistasMes.length === 1 ? "um desejo" : `${conquistasMes.length} desejos`, " neste m\xEAs.")), /* @__PURE__ */ React.createElement("div", { className: "filter-row" }, /* @__PURE__ */ React.createElement("p", { className: "section-intro", style: { margin: 0 } }, "Itens aqui n\xE3o afetam seu saldo at\xE9 voc\xEA marcar como comprado."), /* @__PURE__ */ React.createElement("div", { className: "spacer" }), /* @__PURE__ */ React.createElement("button", { className: "btn-primary sm", onClick: () => setModal("addWish") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), "Desejo")), /* @__PURE__ */ React.createElement("div", { className: "wish-grid" }, pending.length === 0 ? /* @__PURE__ */ React.createElement(EmptyState, { icon: /* @__PURE__ */ React.createElement(Heart, { size: 26 }), text: "Sua lista de desejos est\xE1 vazia." }) : pending.map((w) => /* @__PURE__ */ React.createElement("div", { key: w.id, className: "wish-card" }, /* @__PURE__ */ React.createElement("div", { className: "wish-top" }, /* @__PURE__ */ React.createElement(Gift, { size: 18 }), /* @__PURE__ */ React.createElement(Badge, { tone: w.priority === "alta" ? "pink" : w.priority === "m\xE9dia" ? "orange" : "neutral" }, w.priority)), /* @__PURE__ */ React.createElement("strong", { className: "wish-name" }, w.name), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, w.category), /* @__PURE__ */ React.createElement("div", { className: "wish-value" }, fmt(w.value)), /* @__PURE__ */ React.createElement("div", { className: "wish-actions" }, /* @__PURE__ */ React.createElement("button", { className: "btn-primary sm", onClick: () => setModal({ kind: "buyWish", wish: w }) }, "Comprei"), /* @__PURE__ */ React.createElement("button", { className: "icon-btn", title: "Editar", onClick: () => setModal({ kind: "editWish", wish: w }) }, /* @__PURE__ */ React.createElement(Edit3, { size: 14 })), /* @__PURE__ */ React.createElement("button", { className: "icon-btn danger", onClick: () => onDelete(w.id) }, /* @__PURE__ */ React.createElement(Trash2, { size: 14 })))))), bought.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "J\xE1 comprados")), /* @__PURE__ */ React.createElement("ul", { className: "movement-list" }, bought.map((w) => /* @__PURE__ */ React.createElement("li", { key: w.id }, /* @__PURE__ */ React.createElement("div", { className: "mv-icon saida" }, /* @__PURE__ */ React.createElement(Check, { size: 14 })), /* @__PURE__ */ React.createElement("div", { className: "mv-info" }, /* @__PURE__ */ React.createElement("span", { className: "mv-desc" }, w.name), /* @__PURE__ */ React.createElement("span", { className: "mv-cat" }, w.category)), /* @__PURE__ */ React.createElement("span", { className: "mv-value saida" }, fmt(w.value)))))));
}
function GoalsView({ goals, onDelete, openModal }) {
  const [expanded, setExpanded] = useState(null);
  return /* @__PURE__ */ React.createElement("div", { className: "stack-lg" }, /* @__PURE__ */ React.createElement("div", { className: "filter-row" }, /* @__PURE__ */ React.createElement("div", { className: "spacer" }), /* @__PURE__ */ React.createElement("button", { className: "btn-primary sm", onClick: () => openModal("addGoal") }, /* @__PURE__ */ React.createElement(Plus, { size: 14 }), "Nova meta")), /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Para destinar dinheiro a uma meta, registre uma Sa\xEDda em Lan\xE7amentos e escolha \u201CMeta\u201D em \u201CDestinar para\u201D."), /* @__PURE__ */ React.createElement("div", { className: "goal-grid" }, goals.length === 0 ? /* @__PURE__ */ React.createElement(EmptyState, { icon: /* @__PURE__ */ React.createElement(Target, { size: 26 }), text: "Crie sua primeira meta financeira." }) : goals.map((g) => {
  const pct = Math.min(100, g.current / g.target * 100);
  const monthsLeft = Math.max(1, Math.round((new Date(g.deadline) - /* @__PURE__ */ new Date()) / (1e3 * 60 * 60 * 24 * 30)));
  const monthly = Math.max(0, (g.target - g.current) / monthsLeft);
  const hist = [...(g.history || [])].sort((a, b) => new Date(b.date) - new Date(a.date));
  const isOpen = expanded === g.id;
  return /* @__PURE__ */ React.createElement("div", { key: g.id, className: "goal-card" }, /* @__PURE__ */ React.createElement("div", { className: "goal-card-head" }, /* @__PURE__ */ React.createElement("strong", null, g.name), /* @__PURE__ */ React.createElement("div", { className: "goal-card-actions" }, /* @__PURE__ */ React.createElement(Badge, { tone: "purple" }, Math.round(pct), "%"), /* @__PURE__ */ React.createElement("button", { className: "icon-btn", title: "Editar meta", onClick: () => openModal({ kind: "editGoal", goal: g }) }, /* @__PURE__ */ React.createElement(Edit3, { size: 12 })), /* @__PURE__ */ React.createElement("button", { className: "icon-btn danger", title: "Excluir meta", onClick: () => { if (window.confirm(`Excluir a meta ${g.name}? Isso n\xE3o afeta os lan\xE7amentos que j\xE1 foram feitos, s\xF3 remove a meta.`)) onDelete(g.id); } }, /* @__PURE__ */ React.createElement(Trash2, { size: 12 })))), /* @__PURE__ */ React.createElement(ProgressBar, { value: g.current, max: g.target, colorVar: "linear-gradient(90deg,var(--accent-purple),var(--accent-pink))", height: 10 }), /* @__PURE__ */ React.createElement("div", { className: "goal-numbers" }, /* @__PURE__ */ React.createElement("span", null, fmt(g.current), " de ", fmt(g.target)), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, "at\xE9 ", new Date(g.deadline).toLocaleDateString("pt-BR", { month: "short", year: "numeric" }))), /* @__PURE__ */ React.createElement("div", { className: "goal-suggestion" }, "Guarde ", /* @__PURE__ */ React.createElement("strong", null, fmt(monthly)), "/m\xEAs para atingir no prazo."), /* @__PURE__ */ React.createElement("button", { className: "goal-history-toggle", onClick: () => setExpanded(isOpen ? null : g.id) }, isOpen ? "Ocultar hist\xF3rico" : `Ver hist\xF3rico (${hist.length})`), isOpen && (hist.length === 0 ? /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Nenhuma contribui\xE7\xE3o registrada ainda.") : /* @__PURE__ */ React.createElement("ul", { className: "goal-history-list" }, hist.map((h, i) => /* @__PURE__ */ React.createElement("li", { key: i }, /* @__PURE__ */ React.createElement("span", null, new Date(h.date + "T00:00:00").toLocaleDateString("pt-BR")), /* @__PURE__ */ React.createElement("strong", null, fmt(h.value)))))));
})));
}
function HistoryView({ entries, purchases, debts, savings, investments, expenseCategories, incomeCategories, categoryColors }) {
  ...
  const all = [
    ...entries.map((e) => ({ date: e.date, desc: e.desc, category: e.category, value: e.value, type: e.type, status: e.status, method: e.method || "\u2014" })),
    ...purchases.flatMap((p) => p.installments.map((i) => ({ date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, desc: `${p.desc} (parcela ${i.n}/${p.count})`, category: p.category, value: i.value, type: "saida", status: i.status, method: "Cr\xE9dito" }))),
    ...debts.flatMap((d) => d.installments.map((i) => ({ date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, desc: `${d.name} (parcela ${i.n}/${d.count})`, category: "D\xEDvidas", value: i.value, type: "saida", status: i.status, method: "D\xEDvida" }))),
  ...
  const filtered = all.filter((h) => {
    ...
    if (methodFilter !== "todas" && h.method !== methodFilter) return false;
    if (searchTerm && !(h.desc || "").toLowerCase().includes(searchTerm.toLowerCase())) return false;
    return true;
  }).sort((a, b) => {
  ...
  const hasDateFilter = dateFrom || dateTo || statusFilter !== "todos" || catFilter !== "todas" || methodFilter !== "todas" || searchTerm;
  const allMethodOptions = Array.from(new Set(all.map((h) => h.method).filter(Boolean))).sort();
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "stack-lg" }, /* @__PURE__ */ React.createElement("div", { className: "filter-row" }, /* @__PURE__ */ React.createElement(Field, { label: "De" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: dateFrom, onChange: (e) => setDateFrom(e.target.value) })), /* @__PURE__ */ React.createElement(Field, { label: "At\xE9" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: dateTo, onChange: (e) => setDateTo(e.target.value) })), /* @__PURE__ */ React.createElement(Field, { label: "Situa\xE7\xE3o" }, /* @__PURE__ */ React.createElement("select", { value: statusFilter, onChange: (e) => setStatusFilter(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "todos" }, "Todas"), /* @__PURE__ */ React.createElement("option", { value: "pago" }, "Pago"), /* @__PURE__ */ React.createElement("option", { value: "pendente" }, "Pendente"))), /* @__PURE__ */ React.createElement(Field, { label: "Categoria" }, /* @__PURE__ */ React.createElement("select", { value: catFilter, onChange: (e) => setCatFilter(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "todas" }, "Todas"), allCategoryOptions.map((c) => /* @__PURE__ */ React.createElement("option", { key: c, value: c }, c)))), /* @__PURE__ */ React.createElement(Field, { label: "Forma de pagamento" }, /* @__PURE__ */ React.createElement("select", { value: methodFilter, onChange: (e) => setMethodFilter(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "todas" }, "Todas"), allMethodOptions.map((m) => /* @__PURE__ */ React.createElement("option", { key: m, value: m }, m)))), /* @__PURE__ */ React.createElement(Field, { label: "Buscar" }, /* @__PURE__ */ React.createElement("input", { value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), placeholder: "Buscar pela descri\xE7\xE3o..." })), /* @__PURE__ */ React.createElement("div", { className: "spacer" }), hasDateFilter && /* @__PURE__ */ React.createElement("button", { className: "chip static clear-filters", onClick: () => {
    setDateFrom("");
    setDateTo("");
    setStatusFilter("todos");
    setCatFilter("todas");
...
function TipsView({ totals, categorySpendMonth, budgets }) {
  ...
  const generalTips = [
    ...
    'O card "Como ser\xE1 meu m\xEAs?" na tela inicial mostra o previsto para entrar, o previsto para sair e o quanto deve sobrar no m\xEAs, com base no que voc\xEA j\xE1 cadastrou.',
    "Compras no cart\xE3o de cr\xE9dito n\xE3o descontam o saldo na hora \u2014 elas comprometem o limite do cart\xE3o e s\xF3 afetam o saldo quando voc\xEA marcar a fatura como paga.",
    'Em "Guardados e Investidos", voc\xEA escolhe se cada valor desconta do saldo ou n\xE3o \u2014 \xFAtil para separar dinheiro que j\xE1 estava fora do seu controle di\xE1rio.',
    'A \xE1rea "Pr\xF3ximos Compromissos" reúne contas, parcelas e faturas que ainda v\xE3o vencer. O que j\xE1 passou da data some da lista automaticamente quando voc\xEA marca como pago em Lan\xE7amentos.',
    "Use a lupinha no topo de qualquer tela pra achar rapidinho uma categoria, um cart\xE3o, uma meta ou um lan\xE7amento espec\xEDfico.",
    ...
    "Em Configura\xE7\xF5es \u2192 Apar\xEAncia, voc\xEA pode trocar entre os temas Lavanda, Grafite e Escuro a qualquer momento."
  ];
  const faq = [
    { q: "Por que meu saldo n\xE3o mudou quando fiz uma compra no cart\xE3o?", a: "Porque compras no cr\xE9dito comprometem o limite do cart\xE3o, n\xE3o o seu saldo dispon\xEDvel. O saldo s\xF3 \xE9 afetado quando voc\xEA paga a fatura." },
    { q: "Como marco um lan\xE7amento como pago?", a: "Em Lan\xE7amentos, clique no status (pendente/pago) da linha desejada. Isso atualiza automaticamente o Hist\xF3rico e Pr\xF3ximos Compromissos." },
    { q: "Meus dados ficam salvos se eu trocar de celular?", a: "Sim. Tudo fica sincronizado na nuvem com a sua conta \u2014 basta fazer login no novo aparelho." },
    { q: "Marcar uma parcela como paga afeta as outras parcelas?", a: "N\xE3o. Cada parcela ou ocorr\xEAncia \xE9 independente \u2014 marcar uma como paga nunca altera as demais." },
    { q: "Qual a diferen\xE7a entre Guardado e Investido?", a: "Guardado \xE9 dinheiro parado (reserva, poupan\xE7a). Investido \xE9 dinheiro aplicado, rendendo (CDB, a\xE7\xF5es etc.)." }
  ];
  return /* @__PURE__ */ React.createElement("div", { className: "stack-lg" }, /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "Dicas para voc\xEA")), /* @__PURE__ */ React.createElement("ul", { className: "tips-list" }, tips.map((t, i) => /* @__PURE__ */ React.createElement("li", { key: i }, /* @__PURE__ */ React.createElement(Lightbulb, { size: 16 }), /* @__PURE__ */ React.createElement("span", null, t))))), /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "Dicas do Dashboard"), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, "como aproveitar melhor o Plamily")), /* @__PURE__ */ React.createElement("ul", { className: "tips-list" }, generalTips.map((t, i) => /* @__PURE__ */ React.createElement("li", { key: i }, /* @__PURE__ */ React.createElement(Lightbulb, { size: 16 }), /* @__PURE__ */ React.createElement("span", null, t))))), /* @__PURE__ */ React.createElement("div", { className: "card" }, /* @__PURE__ */ React.createElement("div", { className: "card-head" }, /* @__PURE__ */ React.createElement("h4", null, "D\xFAvidas Frequentes")), /* @__PURE__ */ React.createElement("div", { className: "faq-list" }, faq.map((f, i) => /* @__PURE__ */ React.createElement("details", { key: i, className: "faq-item" }, /* @__PURE__ */ React.createElement("summary", null, f.q), /* @__PURE__ */ React.createElement("p", null, f.a))))));
}
function CategoriesAndCardsView({ cards, expenseCategories, incomeCategories, categoryColors, paymentMethods, debtTypes, banks, onAddCategory, onDeleteCategory, onRenameCategory, onSetCategoryColor, onEditCard, onDeleteCard, openModal }) {
...
function EntryModal({ type, expenseCategories, incomeCategories, paymentMethods, goals, savings, investments, savingPots, investPots, onAddSavingPot, onAddInvestPot, cards, banks, initial, onClose, onSave }) {
  ...
  const [cardId, setCardId] = useState((cards && cards[0] && cards[0].id) || "");
  const [kind, setKind] = useState("unica");
  const [installmentCount, setInstallmentCount] = useState(2);
  const [frequency, setFrequency] = useState("mensal");
  const [weekdays, setWeekdays] = useState([]);
  const [monthlyMode, setMonthlyMode] = useState("fixedDay");
  const [monthDay, setMonthDay] = useState(1);
  const [businessDayN, setBusinessDayN] = useState(5);
  const [annualMonth, setAnnualMonth] = useState(0);
  const [annualDay, setAnnualDay] = useState(1);
  const [endMode, setEndMode] = useState("indefinite");
  ...
  const canSave = desc.trim() && value && Number(value) > 0 && (destino !== "meta" || goalId) && (kind !== "recorrente" || frequency !== "semanal" || weekdays.length > 0);
  const handleSave = () => {
    const recur = { frequency, weekdays, monthlyMode, monthDay: Number(monthDay), businessDayN: Number(businessDayN), annualMonth: Number(annualMonth), annualDay: Number(annualDay), endMode, endDate, endCount: Number(endCount), installmentCount: Number(installmentCount) };
    const useCard = type === "saida" && !isEditing && method === "Cr\xE9dito" && (kind === "unica" || kind === "parcelada" || kind === "recorrente") && cards && cards.length > 0;
    onSave({ type, desc: desc.trim(), category, value: Number(value), date, method, bank, status, destino: type === "saida" && !isEditing ? destino : "nenhum", goalId: destino === "meta" ? goalId : void 0, kind: !isEditing ? kind : "unica", recur, cardId: useCard ? cardId : void 0 });
  };
return /* @__PURE__ */ React.createElement(Modal, { title: isEditing ? "Editar lan\xE7amento" : type === "entrada" ? "Nova entrada" : "Nova sa\xEDda", onClose }, /* @__PURE__ */ React.createElement(Field, { label: "Descri\xE7\xE3o" }, /* @__PURE__ */ React.createElement("input", { value: desc, onChange: (e) => setDesc(e.target.value), placeholder: type === "entrada" ? "Ex: Sal\xE1rio, Freelance..." : "Ex: Supermercado, Uber...", autoFocus: true })), /* @__PURE__ */ React.createElement(Field, { label: "Categoria" }, /* @__PURE__ */ React.createElement("select", { value: category, onChange: (e) => setCategory(e.target.value) }, categories.map((c) => /* @__PURE__ */ React.createElement("option", { key: c }, c)))), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: kind === "parcelada" ? "Valor de cada parcela" : "Valor" }, /* @__PURE__ */ React.createElement("input", { type: "number", value: value, onChange: (e) => setValue(e.target.value), placeholder: "0,00" })), /* @__PURE__ */ React.createElement(Field, { label: kind === "unica" ? "Data" : "Primeira ocorr\xEAncia" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: date, onChange: (e) => setDate(e.target.value) }))), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Forma de pagamento" }, /* @__PURE__ */ React.createElement("select", { value: method, onChange: (e) => { const v = e.target.value; setMethod(v); if (v === "Guardado (retirar)") { setDestino("retirar_guardado"); if (savings && savings[0]) setGoalId(savings[0].id); } else if (v === "Investido (retirar)") { setDestino("retirar_investimento"); if (investments && investments[0]) setGoalId(investments[0].id); } else if (destino === "retirar_guardado" || destino === "retirar_investimento") { setDestino("nenhum"); } } }, paymentMethods.map((m) => /* @__PURE__ */ React.createElement("option", { key: m }, m)), type === "saida" && !isEditing && kind === "unica" && /* @__PURE__ */ React.createElement("option", { value: "Guardado (retirar)" }, "Guardado (retirar)"), type === "saida" && !isEditing && kind === "unica" && /* @__PURE__ */ React.createElement("option", { value: "Investido (retirar)" }, "Investido (retirar)"))), type === "saida" && !isEditing && method === "Cr\xE9dito" && (kind === "unica" || kind === "parcelada" || kind === "recorrente") && (cards && cards.length > 0 ? /* @__PURE__ */ React.createElement(Field, { label: "Qual cart\xE3o?" }, /* @__PURE__ */ React.createElement("select", { value: cardId, onChange: (e) => setCardId(e.target.value) }, cards.map((c) => /* @__PURE__ */ React.createElement("option", { key: c.id, value: c.id }, c.name)))) : /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Voc\xEA ainda n\xE3o tem nenhum cart\xE3o cadastrado. Cadastre um em Categorias primeiro.")), type === "saida" && !isEditing && method === "Cr\xE9dito" && (kind === "unica" || kind === "parcelada" || kind === "recorrente") && cards && cards.length > 0 && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Essa sa\xEDda vai comprometer o limite do cart\xE3o e entrar na fatura certa automaticamente, sem descontar o saldo agora."), /* @__PURE__ */ React.createElement(Field, { label: "Banco / Conta" }, /* @__PURE__ */ React.createElement("select", { value: bank, onChange: (e) => setBank(e.target.value) }, banks.map((b) => /* @__PURE__ */ React.createElement("option", { key: b }, b))))), /* @__PURE__ */ React.createElement(Field, { label: "Status" }, /* @__PURE__ */ React.createElement("div", { className: "seg-toggle" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${status === "pago" ? "sel" : ""}`, onClick: () => setStatus("pago") }, "Pago"), /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${status === "pendente" ? "sel" : ""}`, onClick: () => setStatus("pendente") }, "Pendente"))), !isEditing && /* @__PURE__ */ React.createElement(Field, { label: "Tipo de lan\xE7amento" }, /* @__PURE__ */ React.createElement("div", { className: "seg-toggle" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${kind === "unica" ? "sel" : ""}`, onClick: () => setKind("unica") }, "\xDAnica"), /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${kind === "parcelada" ? "sel" : ""}`, onClick: () => setKind("parcelada") }, "Parcelada"), /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${kind === "recorrente" ? "sel" : ""}`, onClick: () => setKind("recorrente") }, "Recorrente"))), kind === "parcelada" && !isEditing && /* @__PURE__ */ React.createElement(Field, { label: "N\xFAmero de parcelas" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 2, max: 60, value: installmentCount, onChange: (e) => setInstallmentCount(e.target.value) })), kind === "recorrente" && !isEditing && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Field, { label: "Frequ\xEAncia" }, /* @__PURE__ */ React.createElement("select", { value: frequency, onChange: (e) => setFrequency(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "diaria" }, "Di\xE1ria"), /* @__PURE__ */ React.createElement("option", { value: "semanal" }, "Semanal"), /* @__PURE__ */ React.createElement("option", { value: "mensal" }, "Mensal"), /* @__PURE__ */ React.createElement("option", { value: "anual" }, "Anual"))), frequency === "semanal" && /* @__PURE__ */ React.createElement(Field, { label: "Em quais dias?" }, /* @__PURE__ */ React.createElement("div", { className: "weekday-picker" }, ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "S\xE1b"].map((lbl, idx) => /* @__PURE__ */ React.createElement("button", { key: idx, type: "button", className: `weekday-btn ${weekdays.includes(idx) ? "sel" : ""}`, onClick: () => toggleWeekday(idx) }, lbl)))), frequency === "mensal" && /* @__PURE__ */ React.createElement(Field, { label: "Como definir o dia?" }, /* @__PURE__ */ React.createElement("div", { className: "seg-toggle" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${monthlyMode === "fixedDay" ? "sel" : ""}`, onClick: () => setMonthlyMode("fixedDay") }, "Dia fixo"), /* @__PURE__ */ React.createElement("button", { type: "button", className: `seg-btn ${monthlyMode === "businessDay" ? "sel" : ""}`, onClick: () => setMonthlyMode("businessDay") }, "Dia \xDAtil"))), monthlyMode === "businessDay" && frequency === "mensal" && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "A data exata desse dia \xFAtil \xE9 recalculada automaticamente em cada m\xEAs (pula s\xE1bados e domingos)."), frequency === "mensal" && monthlyMode === "fixedDay" && /* @__PURE__ */ React.createElement(Field, { label: "Todo dia" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 1, max: 31, value: monthDay, onChange: (e) => setMonthDay(e.target.value) })), frequency === "mensal" && monthlyMode === "businessDay" && /* @__PURE__ */ React.createElement(Field, { label: "Qual Dia \xDAtil do m\xEAs" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 1, max: 23, value: businessDayN, onChange: (e) => setBusinessDayN(e.target.value) })), frequency === "anual" && /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "M\xEAs" }, /* @__PURE__ */ React.createElement("select", { value: annualMonth, onChange: (e) => setAnnualMonth(e.target.value) }, MONTHS.map((mn, idx) => /* @__PURE__ */ React.createElement("option", { key: idx, value: idx }, mn)))), /* @__PURE__ */ React.createElement(Field, { label: "Dia" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 1, max: 31, value: annualDay, onChange: (e) => setAnnualDay(e.target.value) }))), /* @__PURE__ */ React.createElement(Field, { label: "Dura\xE7\xE3o da recorr\xEAncia" }, /* @__PURE__ */ React.createElement("select", { value: endMode, onChange: (e) => setEndMode(e.target.value) }, /* @__PURE__ */ React.createElement("option", { value: "indefinite" }, "Sem prazo definido"), /* @__PURE__ */ React.createElement("option", { value: "date" }, "At\xE9 uma data"), /* @__PURE__ */ React.createElement("option", { value: "count" }, "N\xFAmero de ocorr\xEAncias"))), endMode === "date" && /* @__PURE__ */ React.createElement(Field, { label: "At\xE9 quando" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: endDate, onChange: (e) => setEndDate(e.target.value) })), endMode === "count" && /* @__PURE__ */ React.createElement(Field, { label: "Quantas vezes" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 1, max: 200, value: endCount, onChange: (e) => setEndCount(e.target.value) }))), type === "saida" && !isEditing && kind === "unica" && /* @__PURE__ */ React.createElement(Field, { label: "Destinar para (opcional)" }, /* @__PURE__ */ React.createElement("select", { value: destino, onChange: (e) => { const v = e.target.value; setDestino(v); if (v === "retirar_guardado" && savingPots && savingPots[0]) setGoalId(savingPots[0].id); else if (v === "retirar_investimento" && investPots && investPots[0]) setGoalId(investPots[0].id); else if (v === "guardado" && savingPots && savingPots[0]) setGoalId(savingPots[0].id); else if (v === "investimento" && investPots && investPots[0]) setGoalId(investPots[0].id); else if (v === "meta" && goals && goals[0]) setGoalId(goals[0].id); } }, /* @__PURE__ */ React.createElement("option", { value: "nenhum" }, "Nenhum — só registrar saída"), /* @__PURE__ */ React.createElement("option", { value: "guardado" }, "Guardado"), /* @__PURE__ */ React.createElement("option", { value: "investimento" }, "Investido"), /* @__PURE__ */ React.createElement("option", { value: "meta" }, "Meta"), /* @__PURE__ */ React.createElement("option", { value: "retirar_guardado" }, "Retirar de um Guardado"), /* @__PURE__ */ React.createElement("option", { value: "retirar_investimento" }, "Retirar de um Investido"))), destino === "meta" && (goals && goals.length > 0 ? /* @__PURE__ */ React.createElement(Field, { label: "Qual meta?" }, /* @__PURE__ */ React.createElement("select", { value: goalId, onChange: (e) => setGoalId(e.target.value) }, goals.map((g) => /* @__PURE__ */ React.createElement("option", { key: g.id, value: g.id }, g.name)))) : /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Você ainda não tem nenhuma meta criada. Crie uma na aba Metas primeiro.")), destino === "guardado" && (savingPots && savingPots.length > 0 ? /* @__PURE__ */ React.createElement(Field, { label: "Em qual cofrinho?" }, /* @__PURE__ */ React.createElement("select", { value: goalId, onChange: (e) => setGoalId(e.target.value) }, savingPots.map((p) => /* @__PURE__ */ React.createElement("option", { key: p.id, value: p.id }, p.name)))) : /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Voc\xEA ainda n\xE3o tem nenhum cofrinho. Crie um em Guardados e Investidos primeiro (ou pelo bot\xE3o \u201CGuardar dinheiro\u201D).")), destino === "investimento" && (investPots && investPots.length > 0 ? /* @__PURE__ */ React.createElement(Field, { label: "Em qual cofrinho?" }, /* @__PURE__ */ React.createElement("select", { value: goalId, onChange: (e) => setGoalId(e.target.value) }, investPots.map((p) => /* @__PURE__ */ React.createElement("option", { key: p.id, value: p.id }, p.name)))) : /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Voc\xEA ainda n\xE3o tem nenhum cofrinho de investimento. Crie um em Guardados e Investidos primeiro.")), destino === "retirar_guardado" && (savingPots && savingPots.length > 0 ? /* @__PURE__ */ React.createElement(Field, { label: "De qual cofrinho?" }, /* @__PURE__ */ React.createElement("select", { value: goalId, onChange: (e) => setGoalId(e.target.value) }, savingPots.map((p) => /* @__PURE__ */ React.createElement("option", { key: p.id, value: p.id }, p.name, " (", fmt(savings.filter((s) => s.potId === p.id).reduce((sum, s) => sum + s.value, 0)), ")")))) : /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Voc\xEA ainda n\xE3o tem nenhum cofrinho de guardado. Crie um primeiro em Guardados e Investidos.")), destino === "retirar_investimento" && (investPots && investPots.length > 0 ? /* @__PURE__ */ React.createElement(Field, { label: "De qual cofrinho?" }, /* @__PURE__ */ React.createElement("select", { value: goalId, onChange: (e) => setGoalId(e.target.value) }, investPots.map((p) => /* @__PURE__ */ React.createElement("option", { key: p.id, value: p.id }, p.name, " (", fmt(investments.filter((i) => i.potId === p.id).reduce((sum, i) => sum + i.value, 0)), ")")))) : /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Voc\xEA ainda n\xE3o tem nenhum cofrinho de investimento. Crie um primeiro em Guardados e Investidos.")), destino !== "nenhum" && destino !== "retirar_guardado" && destino !== "retirar_investimento" && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Esse valor já será descontado do saldo como saída, e também vai aparecer em Guardados e Investidos ou na sua Meta."), (destino === "retirar_guardado" || destino === "retirar_investimento") && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Essa saída também vai descontar o valor escolhido do cofrinho — deixando quanto sobrou lá."), isEditing && (initial && initial.fixed) && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Este lan\xE7amento faz parte de uma recorr\xEAncia. Editar aqui muda somente esta ocorr\xEAncia."), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !canSave, onClick: handleSave }, isEditing ? "Salvar altera\xE7\xF5es" : kind === "parcelada" ? "Criar parcelas" : kind === "recorrente" ? "Criar recorr\xEAncia" : "Adicionar"))}

function CardModal({ card, banks, onClose, onSave }) {
  ...
  return /* @__PURE__ */ React.createElement(Modal, { title: card ? "Editar cart\xE3o" : "Novo cart\xE3o", onClose }, /* @__PURE__ */ React.createElement(Field, { label: "Nome do cart\xE3o" }, /* @__PURE__ */ React.createElement("input", { value: name, onChange: (e) => setName(e.target.value), placeholder: "Ex: Nubank Roxinho" })), /* @__PURE__ */ React.createElement(Field, { label: "Banco" }, /* @__PURE__ */ React.createElement("select", { value: bank, onChange: (e) => setBank(e.target.value) }, banks.map((b) => /* @__PURE__ */ React.createElement("option", { key: b }, b)))), /* @__PURE__ */ React.createElement(Field, { label: "Limite" }, /* @__PURE__ */ React.createElement("input", { type: "number", value: limit, onChange: (e) => setLimit(e.target.value), placeholder: "0,00" })), /* @__PURE__ */ React.createElement(Field, { label: "Cor" }, /* @__PURE__ */ React.createElement("div", { className: "color-pick" }, CARD_COLORS.map((c) => /* @__PURE__ */ React.createElement("button", { key: c, className: `color-dot ${color === c ? "sel" : ""}`, style: { background: c }, onClick: () => setColor(c) })), /* @__PURE__ */ React.createElement("label", { className: "cat-color-label lg", title: "Escolher outra cor", style: { width: 26, height: 26 } }, /* @__PURE__ */ React.createElement("input", { type: "color", value: color, onChange: (e) => setColor(e.target.value), className: "cat-color-input" }), /* @__PURE__ */ React.createElement("span", { className: "color-dot custom-color-dot", style: { background: color } }, /* @__PURE__ */ React.createElement(Plus, { size: 12 }))))), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Fechamento (dia)" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 1, max: 31, value: closing, onChange: (e) => setClosing(Number(e.target.value)) })), /* @__PURE__ */ React.createElement(Field, { label: "Vencimento (dia)" }, /* @__PURE__ */ React.createElement("input", { type: "number", min: 1, max: 31, value: due, onChange: (e) => setDue(Number(e.target.value)) }))), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !name.trim() || !limit, onClick: () => onSave({ name: name.trim(), bank, limit: Number(limit), color, closing, due }) }, card ? "Salvar altera\xE7\xF5es" : "Adicionar cart\xE3o"));
}
function DebtModal({ debtTypes, banks, onClose, onSave }) {
...
function ConfirmDangerousModal({ action, userEmail, onClose, onConfirm }) {
  var _a, _b;
  const providerId = (_b = (_a = auth.currentUser) == null ? void 0 : _a.providerData[0]) == null ? void 0 : _b.providerId;
  const isGoogle = providerId && providerId !== "password";
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [typed, setTyped] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const word = action === "delete" ? "EXCLUIR" : "REINICIAR";
  const title = action === "delete" ? "Excluir conta" : "Reiniciar conta";
  const warning = action === "delete" ? "Essa a\xE7\xE3o \xE9 permanente. Todos os seus dados ser\xE3o apagados e n\xE3o podem ser recuperados." : "Todos os seus dados ser\xE3o zerados, mas sua conta e seu login continuam os mesmos.";
  const handleConfirm = async () => {
    setError("");
    if (isGoogle) {
      if (typed.trim().toUpperCase() !== word) {
        setError(`Digite ${word} certinho, em mai\xFAsculas, para confirmar.`);
        return;
      }
      setLoading(true);
      try {
        ...
        setError("N\xE3o foi poss\xEDvel confirmar com o Google. Tente novamente.");
        setLoading(false);
      }
    } else {
      if (!password) {
  ...
  return /* @__PURE__ */ React.createElement(Modal, { title, onClose }, /* @__PURE__ */ React.createElement("div", { className: `confirm-danger-icon ${action === "delete" ? "is-delete" : "is-reset"}` }, /* @__PURE__ */ React.createElement(AlertCircle, { size: 22 })), /* @__PURE__ */ React.createElement("p", { className: "section-intro" }, warning), isGoogle ? /* @__PURE__ */ React.createElement(Field, { label: `Digite ${word} para confirmar` }, /* @__PURE__ */ React.createElement("input", { value: typed, onChange: (e) => setTyped(e.target.value), placeholder: word, autoFocus: true })) : /* @__PURE__ */ React.createElement(Field, { label: "Sua senha" }, /* @__PURE__ */ React.createElement("div", { className: "password-field-wrap" }, /* @__PURE__ */ React.createElement("input", { type: showPassword ? "text" : "password", value: password, onChange: (e) => setPassword(e.target.value), autoFocus: true }), /* @__PURE__ */ React.createElement("button", { type: "button", className: "password-eye-btn", onClick: () => setShowPassword((v) => !v) }, showPassword ? /* @__PURE__ */ React.createElement(EyeOff, { size: 16 }) : /* @__PURE__ */ React.createElement(Eye, { size: 16 })))), error && /* @__PURE__ */ React.createElement("div", { className: "auth-error" }, /* @__PURE__ */ React.createElement(AlertCircle, { size: 14 }), error), /* @__PURE__ */ React.createElement("button", { className: `btn-primary full ${action === "delete" ? "btn-danger-solid" : ""}`, disabled: loading, onClick: handleConfirm }, loading ? "Confirmando\u2026" : action === "delete" ? "Excluir definitivamente" : "Reiniciar conta"));
}
...
function EditDebtModal({ debt, debtTypes, banks, onClose, onSave }) {
...
function WishModal({ initial, categories, onClose, onSave }) {
  const [name, setName] = useState((initial && initial.name) || "");
  const [value, setValue] = useState((initial && initial.value) || "");
  const [category, setCategory] = useState((initial && initial.category) || categories[0]);
  const [priority, setPriority] = useState((initial && initial.priority) || "m\xE9dia");
  const isEditing = !!initial;
  return /* @__PURE__ */ React.createElement(Modal, { title: isEditing ? "Editar desejo" : "Adicionar \xE0 lista de desejos", onClose }, /* @__PURE__ */ React.createElement(Field, { label: "Item" }, /* @__PURE__ */ React.createElement("input", { value: name, onChange: (e) => setName(e.target.value), placeholder: "Ex: Fone bluetooth", autoFocus: true })), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Valor estimado" }, /* @__PURE__ */ React.createElement("input", { type: "number", value, onChange: (e) => setValue(e.target.value), placeholder: "0,00" })), /* @__PURE__ */ React.createElement(Field, { label: "Categoria" }, /* @__PURE__ */ React.createElement("select", { value: category, onChange: (e) => setCategory(e.target.value) }, categories.map((c) => /* @__PURE__ */ React.createElement("option", { key: c }, c))))), /* @__PURE__ */ React.createElement(Field, { label: "Prioridade" }, /* @__PURE__ */ React.createElement("div", { className: "status-pick" }, ["baixa", "m\xE9dia", "alta"].map((p) => /* @__PURE__ */ React.createElement("button", { key: p, className: priority === p ? "sel" : "", onClick: () => setPriority(p) }, p)))), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !name || !value, onClick: () => onSave({ name, value: Number(value), category, priority }) }, isEditing ? "Salvar altera\xE7\xF5es" : "Adicionar desejo"));
}
function BuyWishModal({ wish, cards, paymentMethods, banks, onClose, onConfirm }) {
  var _a;
...
function OpeningBalanceModal({ current, onClose, onSave }) {
  const [value, setValue] = useState(current || "");
  return /* @__PURE__ */ React.createElement(Modal, { title: "Saldo inicial", onClose }, /* @__PURE__ */ React.createElement("p", { className: "section-intro" }, "Quanto voc\xEA j\xE1 tinha guardado ou na conta quando come\xE7ou a usar o Plamily? Informe aqui e esse valor entra automaticamente no seu saldo, sem precisar criar um lan\xE7amento falso de \u201Centrada\u201D pra representar isso."), /* @__PURE__ */ React.createElement(Field, { label: "Valor que voc\xEA j\xE1 tinha" }, /* @__PURE__ */ React.createElement("input", { type: "number", value, onChange: (e) => setValue(e.target.value), placeholder: "0,00", autoFocus: true })), /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Deixe em 0 se quiser come\xE7ar a contagem do zero. Voc\xEA pode voltar aqui e mudar esse valor quando quiser."), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", onClick: () => { onSave(Number(value) || 0); onClose(); } }, "Salvar saldo inicial"));
}
function TransferModal({ banks, onClose, onSave }) {
  const [fromBank, setFromBank] = useState(banks[0]);
  const [toBank, setToBank] = useState(banks[1] || banks[0]);
  const [value, setValue] = useState("");
  const [date, setDate] = useState(todayISO());
  const canSave = value && Number(value) > 0 && fromBank !== toBank;
  return /* @__PURE__ */ React.createElement(Modal, { title: "Transfer\xEAncia entre contas", onClose }, /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Move dinheiro de uma conta pra outra, sem contar como ganho ou gasto de verdade."), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "De" }, /* @__PURE__ */ React.createElement("select", { value: fromBank, onChange: (e) => setFromBank(e.target.value) }, banks.map((b) => /* @__PURE__ */ React.createElement("option", { key: b }, b)))), /* @__PURE__ */ React.createElement(Field, { label: "Para" }, /* @__PURE__ */ React.createElement("select", { value: toBank, onChange: (e) => setToBank(e.target.value) }, banks.map((b) => /* @__PURE__ */ React.createElement("option", { key: b }, b))))), fromBank === toBank && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Escolha contas diferentes."), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Valor" }, /* @__PURE__ */ React.createElement("input", { type: "number", value, onChange: (e) => setValue(e.target.value), placeholder: "0,00", autoFocus: true })), /* @__PURE__ */ React.createElement(Field, { label: "Data" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: date, onChange: (e) => setDate(e.target.value) }))), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !canSave, onClick: () => onSave({ fromBank, toBank, value: Number(value), date }) }, "Transferir"));
}
function GoalModal({ initial, onClose, onSave }) {
  ...
  const isEditing = !!initial;
  return /* @__PURE__ */ React.createElement(Modal, { title: isEditing ? "Editar meta" : "Nova meta financeira", onClose }, /* @__PURE__ */ React.createElement(Field, { label: "Nome da meta" }, /* @__PURE__ */ React.createElement("input", { value: name, onChange: (e) => setName(e.target.value), placeholder: "Ex: Viagem, Reserva...", autoFocus: true })), /* @__PURE__ */ React.createElement(Field, { label: "Valor objetivo" }, /* @__PURE__ */ React.createElement("input", { type: "number", value: target, onChange: (e) => setTarget(e.target.value), placeholder: "0,00" })), /* @__PURE__ */ React.createElement(Field, { label: "Prazo" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: deadline, onChange: (e) => setDeadline(e.target.value) })), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !name || !target || !deadline, onClick: () => onSave({ name, target: Number(target), deadline }) }, isEditing ? "Salvar altera\xE7\xF5es" : "Criar meta"));
}
function SavingModal({ initial, banks, pots, onAddPot, onClose, onSave }) {
  const [desc, setDesc] = useState((initial && initial.desc) || "");
  const [value, setValue] = useState((initial && initial.value) || "");
  const [date, setDate] = useState((initial && initial.date) || todayISO());
  const [bank, setBank] = useState((initial && initial.bank) || banks[0]);
  ...
  const [potId, setPotId] = useState((initial && initial.potId) || (pots[0] && pots[0].id) || "__new__");
  const [newPotName, setNewPotName] = useState("");
  const isEditing = !!initial;
  const handleSave = () => {
    let finalPotId = potId;
    let finalPotName = "";
    if (potId === "__new__") {
      if (!newPotName.trim()) return;
      finalPotId = onAddPot(newPotName);
      finalPotName = newPotName.trim();
    } else {
      finalPotName = (pots.find((p) => p.id === potId) || {}).name || "";
    }
    onSave({ desc: desc.trim() || finalPotName || "Guardado", value: Number(value), bank, yieldRate: yieldRate ? Number(yieldRate) : null, date, discount, potId: finalPotId });
  };
  return /* @__PURE__ */ React.createElement(Modal, { title: isEditing ? "Editar guardado" : "Guardar dinheiro", onClose }, /* @__PURE__ */ React.createElement(Field, { label: "Descri\xE7\xE3o (opcional)" }, /* @__PURE__ */ React.createElement("input", { value: desc, onChange: (e) => setDesc(e.target.value), placeholder: "Ex: Reserva de emerg\xEAncia", autoFocus: true })), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Valor" }, /* @__PURE__ */ React.createElement("input", { type: "number", value, onChange: (e) => setValue(e.target.value), placeholder: "0,00" })), /* @__PURE__ */ React.createElement(Field, { label: "Data" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: date, onChange: (e) => setDate(e.target.value) }))), /* @__PURE__ */ React.createElement(Field, { label: "Cofrinho" }, /* @__PURE__ */ React.createElement("select", { value: potId, onChange: (e) => setPotId(e.target.value) }, pots.map((p) => /* @__PURE__ */ React.createElement("option", { key: p.id, value: p.id }, p.name)), /* @__PURE__ */ React.createElement("option", { value: "__new__" }, "+ Criar novo cofrinho"))), potId === "__new__" && /* @__PURE__ */ React.createElement(Field, { label: "Nome do cofrinho novo" }, /* @__PURE__ */ React.createElement("input", { value: newPotName, onChange: (e) => setNewPotName(e.target.value), placeholder: "Ex: Viagem, Emerg\xEAncia..." })), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Banco / institui\xE7\xE3o" }, /* @__PURE__ */ React.createElement("select", { value: bank, onChange: (e) => setBank(e.target.value) }, banks.map((b) => /* @__PURE__ */ React.createElement("option", { key: b }, b)))), /* @__PURE__ */ React.createElement(Field, { label: "Rendimento (% a.m., opcional)" }, /* @__PURE__ */ React.createElement("input", { type: "number", step: "0.01", value: yieldRate, onChange: (e) => setYieldRate(e.target.value), placeholder: "Ex: 0,85" }))), !isEditing && /* @__PURE__ */ React.createElement(Field, { label: "Descontar do saldo dispon\xEDvel?" }, /* @__PURE__ */ React.createElement("div", { className: "status-pick" }, /* @__PURE__ */ React.createElement("button", { className: discount ? "sel" : "", onClick: () => setDiscount(true) }, "Sim"), /* @__PURE__ */ React.createElement("button", { className: !discount ? "sel" : "", onClick: () => setDiscount(false) }, "N\xE3o"))), isEditing && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Para mudar se este valor desconta do saldo, exclua e registre novamente."), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !value || (potId === "__new__" && !newPotName.trim()), onClick: handleSave }, isEditing ? "Salvar altera\xE7\xF5es" : "Guardar"));
}
function InvestmentModal({ initial, banks, pots, onAddPot, onClose, onSave }) {
  const [desc, setDesc] = useState((initial && initial.desc) || "");
  const [value, setValue] = useState((initial && initial.value) || "");
  const [date, setDate] = useState((initial && initial.date) || todayISO());
  const [bank, setBank] = useState((initial && initial.bank) || banks[0]);
  ...
  const handleSave = () => {
    ...
    onSave({ desc: desc.trim() || finalPotName || "Investido", value: Number(value), bank, yieldRate: yieldRate ? Number(yieldRate) : null, date, discount, potId: finalPotId });
  };
  return /* @__PURE__ */ React.createElement(Modal, { title: isEditing ? "Editar investimento" : "Registrar investimento", onClose }, /* @__PURE__ */ React.createElement(Field, { label: "Descri\xE7\xE3o (opcional)" }, /* @__PURE__ */ React.createElement("input", { value: desc, onChange: (e) => setDesc(e.target.value), placeholder: "Ex: Tesouro Selic", autoFocus: true })), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Valor" }, /* @__PURE__ */ React.createElement("input", { type: "number", value, onChange: (e) => setValue(e.target.value), placeholder: "0,00" })), /* @__PURE__ */ React.createElement(Field, { label: "Data" }, /* @__PURE__ */ React.createElement("input", { type: "date", value: date, onChange: (e) => setDate(e.target.value) }))), /* @__PURE__ */ React.createElement(Field, { label: "Cofrinho" }, /* @__PURE__ */ React.createElement("select", { value: potId, onChange: (e) => setPotId(e.target.value) }, pots.map((p) => /* @__PURE__ */ React.createElement("option", { key: p.id, value: p.id }, p.name)), /* @__PURE__ */ React.createElement("option", { value: "__new__" }, "+ Criar novo cofrinho"))), potId === "__new__" && /* @__PURE__ */ React.createElement(Field, { label: "Nome do cofrinho novo" }, /* @__PURE__ */ React.createElement("input", { value: newPotName, onChange: (e) => setNewPotName(e.target.value), placeholder: "Ex: Tesouro Direto, A\xE7\xF5es..." })), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement(Field, { label: "Banco / institui\xE7\xE3o" }, /* @__PURE__ */ React.createElement("select", { value: bank, onChange: (e) => setBank(e.target.value) }, banks.map((b) => /* @__PURE__ */ React.createElement("option", { key: b }, b)))), /* @__PURE__ */ React.createElement(Field, { label: "Rendimento (% a.m., opcional)" }, /* @__PURE__ */ React.createElement("input", { type: "number", step: "0.01", value: yieldRate, onChange: (e) => setYieldRate(e.target.value), placeholder: "Ex: 1,05" }))), !isEditing && /* @__PURE__ */ React.createElement(Field, { label: "Descontar do saldo dispon\xEDvel?" }, /* @__PURE__ */ React.createElement("div", { className: "status-pick" }, /* @__PURE__ */ React.createElement("button", { className: discount ? "sel" : "", onClick: () => setDiscount(true) }, "Sim"), /* @__PURE__ */ React.createElement("button", { className: !discount ? "sel" : "", onClick: () => setDiscount(false) }, "N\xE3o"))), isEditing && /* @__PURE__ */ React.createElement("p", { className: "muted-sm" }, "Para mudar se este valor desconta do saldo, exclua e registre novamente."), /* @__PURE__ */ React.createElement("button", { className: "btn-primary full", disabled: !value || (potId === "__new__" && !newPotName.trim()), onClick: handleSave }, isEditing ? "Salvar altera\xE7\xF5es" : "Registrar"));
}
const CSS = `
...
.side-nav {
  display: flex; flex-direction: column; gap: 3px; flex: 1; overflow-y: auto;
  scrollbar-width: thin; scrollbar-color: var(--border) transparent;
}
.side-nav::-webkit-scrollbar { width: 5px; }
.side-nav::-webkit-scrollbar-track { background: transparent; }
.side-nav::-webkit-scrollbar-thumb { background: var(--border); border-radius: 10px; }
.side-nav::-webkit-scrollbar-thumb:hover { background: var(--accent-purple); }
.side-item {
...
.ob-editable-value {
  display: flex; align-items: center; gap: 6px; background: transparent; border: none; padding: 0;
  font-family: 'Fraunces', serif; font-size: 20px; font-weight: 600; color: var(--text-dark); cursor: pointer;
}
.ob-editable-value svg { opacity: .5; }
.ob-edit-row { display: flex; align-items: center; gap: 6px; }
.ob-edit-row input { padding: 6px 8px; font-size: 14px; border-radius: 8px; }
.ob-save-btn { flex-shrink: 0; width: 28px; height: 28px; border-radius: 8px; border: none; background: var(--accent-purple); color: white; display: flex; align-items: center; justify-content: center; cursor: pointer; }
...
.fv-income-block { margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border); display: flex; flex-direction: column; gap: 8px; }
...
.qa-scroll::-webkit-scrollbar { height: 4px; }
.qa-scroll::-webkit-scrollbar-track { background: transparent; }
.qa-scroll::-webkit-scrollbar-thumb { background: var(--accent-purple); border-radius: 10px; }
.qa-btn {
...
.donut-legend-pct { flex: 0 0 auto !important; font-size: 11px; color: var(--text-muted); min-width: 30px; text-align: right; }
...
.weekday-btn {
  ...
  color: var(--text-muted); font-weight: 700; font-size: 11px; cursor: pointer; font-family: inherit;
}
.weekday-btn.sel { background: var(--accent-purple); color: white; border-color: var(--accent-purple); }
.seg-btn {
...
.diag-list { display: flex; flex-direction: column; gap: 8px; margin-top: 8px; }
.diag-row { display: flex; align-items: center; justify-content: space-between; font-size: 12.5px; padding: 8px 10px; background: var(--surface-alt); border-radius: 10px; }
.diag-scope { word-break: break-all; overflow-wrap: anywhere; }
.diag-row strong { font-size: 11.5px; font-weight: 700; }
...
.month-projection-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px 10px; margin: 20px 0 4px; }
...
.hist-summary { display: grid; grid-template-columns: 1fr 1fr; gap: 16px 20px; }
...
.pot-card { border: 1px solid var(--border); border-radius: 14px; padding: 4px 4px 10px; margin-bottom: 10px; }
.pot-head {
  width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 10px;
  border: none; background: transparent; cursor: pointer; padding: 10px 10px; font-family: inherit;
}
.pot-name { display: flex; align-items: center; gap: 8px; font-weight: 700; font-size: 13px; color: var(--text-dark); }
.pot-name svg { color: var(--accent-purple); flex-shrink: 0; }
.pot-total { display: flex; align-items: center; gap: 6px; font-family: 'Fraunces', serif; font-weight: 700; font-size: 14px; color: var(--text-dark); }
.pot-total svg { color: var(--text-muted); transition: transform .15s; }
.pot-actions-row { display: flex; justify-content: flex-end; padding: 0 10px 4px; }
.pot-card .sav-inv-list { padding: 0 10px; }
...
.conquest-item svg { color: var(--accent-purple); flex-shrink: 0; }
...
.due-item-paid-btn {
  display: flex; align-items: center; gap: 4px; border: 1px solid var(--border); background: var(--surface-alt);
  color: var(--text-muted); font-size: 10px; font-weight: 700; padding: 3px 9px; border-radius: 100px; cursor: pointer; font-family: inherit;
}
.due-item-paid-btn:hover { background: var(--green); border-color: var(--green); color: white; }
.confirm-danger-icon {
  width: 46px; height: 46px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 4px;
}
.confirm-danger-icon.is-delete { background: var(--accent-pink-tint); color: var(--danger); }
.confirm-danger-icon.is-reset { background: var(--accent-purple-tint); color: var(--accent-purple); }
.btn-danger-solid { background: var(--danger) !important; }
.btn-danger-solid:hover { filter: brightness(0.92); }
.faq-list { display: flex; flex-direction: column; gap: 8px; }
.faq-item { border: 1px solid var(--border); border-radius: 12px; padding: 12px 14px; background: var(--surface-alt); }
.faq-item summary { cursor: pointer; font-weight: 700; font-size: 12.5px; color: var(--text-dark); list-style: none; }
.faq-item summary::-webkit-details-marker { display: none; }
.faq-item summary::before { content: "+"; display: inline-block; width: 16px; color: var(--accent-purple); font-weight: 700; }
.faq-item[open] summary::before { content: "−"; }
.faq-item p { margin: 8px 0 0 16px; font-size: 12px; color: var(--text-muted); line-height: 1.5; }
.sort-toggle-row { display: flex; gap: 4px; flex-shrink: 0; }
.sort-toggle-row .icon-btn { width: 26px; height: 26px; }
.sort-toggle-row .icon-btn.sel-sort { background: var(--accent-purple); color: white; }
...
.cc-limit-row { display: flex; justify-content: space-between; align-items: baseline; margin-top: 12px; font-size: 10px; opacity: .9; }
.cc-limit-row strong { font-size: 11px; font-weight: 700; }
.cc-limit-bar { height: 4px; background: rgba(255,255,255,.3); border-radius: 4px; margin-top: 6px; overflow: hidden; }
...
.invoice-row {
  ...
  padding: 12px 14px; background: var(--surface-alt); border-radius: 12px;
}
.invoice-info { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.invoice-card-name { font-weight: 700; font-size: 13px; color: var(--text-dark); }
.invoice-actions { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
.invoice-actions strong { font-family: 'Fraunces', serif; font-size: 14px; }
.invoice-row.invoice-overdue { border: 1px solid var(--danger); background: var(--accent-pink-tint); }
...
.goal-history-toggle {
  border: none; background: transparent; color: var(--accent-purple); font-size: 11px; font-weight: 700;
  cursor: pointer; padding: 2px 0; text-align: left; font-family: inherit;
}
.goal-history-list { display: flex; flex-direction: column; gap: 6px; margin-top: 4px; }
.goal-history-list li { display: flex; justify-content: space-between; font-size: 11.5px; color: var(--text-muted); padding: 4px 0; border-bottom: 1px solid var(--border); }
.goal-history-list li strong { color: var(--text-dark); font-family: 'Fraunces', serif; }
...
.modal-panel::-webkit-scrollbar-thumb { background: var(--border); border-radius: 10px; }
.modal-panel::-webkit-scrollbar-thumb:hover { background: var(--accent-purple); }
        ...
        if (u) {
          ...
          try {
            ...
            if (snap.exists()) {
              ...
              if (cloudTheme === "grafite" || cloudTheme === "escuro" || cloudTheme === "lavanda") {
                setTheme(cloudTheme);
              }
            }
          } catch (e) {
