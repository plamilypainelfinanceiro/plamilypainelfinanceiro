...
function FinancePlanner({ accountId, db: db2, userEmail, userName, onSignOut, onUpdateName }) {
  ...
  const upcoming = useMemo(() => {
    const list = [];
    entries.filter((e) => e.status === "pendente").forEach((e) => list.push({ id: e.id, desc: e.desc, value: e.value, date: e.date, type: e.type, kind: "Conta", source: "entry", rawId: e.id }));
