...
function CardsView({ cards, purchases, cardInvoices, onToggleInstallment, onPayInvoice, onReopenInvoice, openModal }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "invoice-list" }, rows.map(({ card, inv }) => /* @__PURE__ */ React.createElement("div", { key: card.id + inv.dueKey, className: "invoice-row" }, /* @__PURE__ */ React.createElement("div", { className: "invoice-info" }, /* @__PURE__ */ React.createElement("span", { className: "invoice-card-name" }, card.name), /* @__PURE__ */ React.createElement("span", { className: "muted-sm" }, MONTHS[inv.dueM], "/", inv.dueY, " \xB7 ", inv.itemCount, " item", inv.itemCount > 1 ? "s" : "", inv.isFullyPaid ? " \xB7 paga" : ` \xB7 ${inv.pendingCount} pendente${inv.pendingCount > 1 ? "s" : ""}`)), /* @__PURE__ */ React.createElement("div", { className: "invoice-actions" }, /* @__PURE__ */ React.createElement("strong", null, fmt(inv.isFullyPaid ? inv.total : inv.pendingTotal)), inv.isFullyPaid ? /* @__PURE__ */ React.createElement("button", { className: "chip static", onClick: () => onReopenInvoice(card.id, inv.dueKey) }, "Reabrir fatura") : /* @__PURE__ */ React.createElement("button", { className: "btn-primary sm", onClick: () => { if (window.confirm(`Marcar toda a fatura de ${MONTHS[inv.dueM]}/${inv.dueY} do ${card.name} como paga? Isso libera ${fmt(inv.pendingTotal)} do limite.`)) onPayInvoice(card.id, inv.dueKey); } }, "Pagar fatura")))));
