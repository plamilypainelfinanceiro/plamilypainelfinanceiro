...
function FinancePlanner({ accountId, db: db2, userEmail, userName, onSignOut, onUpdateName }) {
  ...
  const top5Saidas = useMemo(() => {
    return entries.filter((e) => e.type === "saida" && e.status === "pago" && dateInMonth(e.date, selY, selM)).sort((a, b) => b.value - a.value).slice(0, 5);
  }, [entries, selY, selM]);
...
function AnnualView({ entries, selY }) {
  const monthData = useMemo(() => {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      const totalIn = monthEntries.filter((e) => e.type === "entrada" && e.status === "pago").reduce((s, e) => s + e.value, 0);
      const totalOut = monthEntries.filter((e) => e.type === "saida" && e.status === "pago").reduce((s, e) => s + e.value, 0);
