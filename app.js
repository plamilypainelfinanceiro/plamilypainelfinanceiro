...
function FinancePlanner({ accountId, db: db2, userEmail, userName, onSignOut, onUpdateName }) {
  ...
  const upcoming = useMemo(() => {
    ...
    allInstallmentsFlat.filter((i) => i.status === "pendente").forEach(
      (i) => list.push({ id: i.purchaseId + i.n, desc: `${i.desc} (parcela ${i.n})`, value: i.value, date: `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", kind: "Cart\xE3o", source: "purchase", rawId: i.purchaseId, n: i.n })
    );
    debtInstallmentsFlat.filter((i) => i.status === "pendente").forEach(
      (i) => list.push({ id: i.debtId + i.n, desc: `${i.desc} (parcela ${i.n})`, value: i.value, date: `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, type: "saida", kind: "D\xEDvida", source: "debt", rawId: i.debtId, n: i.n })
    );
