...
function HistoryView({ entries, purchases, debts, savings, investments, expenseCategories, incomeCategories, categoryColors }) {
  ...
  const all = [
    ...
    ...purchases.flatMap((p) => p.installments.map((i) => ({ date: i.dueDate || `${i.dueY}-${String(i.dueM + 1).padStart(2, "0")}-01`, desc: `${p.desc} (parcela ${i.n}/${p.count})`, category: p.category, value: i.value, type: "saida", status: i.status }))),
