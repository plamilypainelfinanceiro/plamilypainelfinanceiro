...
function SettingsView({ onUpdateApp, onExportCSV, openingBalance, onSetOpeningBalance, userName, onUpdateName, profilePhoto, onPhotoUpload, onPhotoRemove }) {
  const { theme, setTheme } = useTheme();
  const [nameInput, setNameInput] = useState(userName || "");
  const [editingName, setEditingName] = useState(false);
  const [swDiag, setSwDiag] = useState(null);
  const [obInput, setObInput] = useState(openingBalance || "");
  const [obDirty, setObDirty] = useState(false);
  useEffect(() => {
    try {
